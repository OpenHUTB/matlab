...
// Source File Name:   ArrayOfArchitecture.java
...
//            Architecture, ExtensionMapper

public class ArrayOfArchitecture
    implements ADBBean
{
    public static class Factory
    {

        public static ArrayOfArchitecture parse(XMLStreamReader reader)
            throws Exception
        {
            ArrayOfArchitecture object;
            object = new ArrayOfArchitecture();
            ...
            try
            {
                ...
                if(reader.getAttributeValue("http://www.w3.org/2001/XMLSchema-instance", "type") != null)
                {
                    ...
                    if(fullTypeName != null)
                    {
                        ...
                        if(!"ArrayOfArchitecture".equals(type))
                        {
                            String nsUri = reader.getNamespaceContext().getNamespaceURI(nsPrefix);
                            return (ArrayOfArchitecture)ExtensionMapper.getTypeObject(nsUri, type, reader);
                        }
                    }
                }
            }
            ...
            {
                list1.add(Architecture.Factory.parse(reader));
                loopDone1 = false;
                while(!loopDone1) 
                {
                ...
                object.setItem((Architecture[])(Architecture[])ConverterUtil.convertToArray(com/mathworks/internal/dws/client/v3/Architecture, list1));
            }
            for(; !reader.isStartElement() && !reader.isEndElement(); reader.next());
    ...
    public Architecture[] getItem()
    {
        return localItem;
    }

    protected void validateItem(Architecture aarchitecture[])
    {
    }

    public void setItem(Architecture param[])
    {
        validateItem(param);
        if(param != null)
    ...
    public void addItem(Architecture param)
    {
        if(localItem == null)
            localItem = new Architecture[0];
        localItemTracker = true;
        List list = ConverterUtil.toList(localItem);
        list.add(param);
        localItem = (Architecture[])(Architecture[])list.toArray(new Architecture[list.size()]);
    }
        ...
        throws ADBException
    {
        org.apache.axiom.om.OMDataSource dataSource = new ADBDataSource(parentQName, factory) {
            ...
            {
                ArrayOfArchitecture.this.serialize(parentQName, factory, xmlWriter);
            }

            final OMFactory val$factory;
            final ArrayOfArchitecture this$0;

            
            {
                this$0 = ArrayOfArchitecture.this;
                factory = omfactory;
                super(x0, x1);
            }
        }
;
        ...
        throws XMLStreamException, ADBException
    {
        ...
        if(serializeType)
        {
            ...
            if(namespacePrefix != null && namespacePrefix.trim().length() > 0)
                writeAttribute("xsi", "http://www.w3.org/2001/XMLSchema-instance", "type", (new StringBuilder()).append(namespacePrefix).append(":ArrayOfArchitecture").toString(), xmlWriter);
            else
                writeAttribute("xsi", "http://www.w3.org/2001/XMLSchema-instance", "type", "ArrayOfArchitecture", xmlWriter);
        }
        if(localItemTracker)
            if(localItem != null)
            {
    ...
    protected Architecture localItem[];

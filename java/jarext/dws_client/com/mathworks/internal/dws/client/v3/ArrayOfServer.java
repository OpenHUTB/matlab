...
// Source File Name:   ArrayOfServer.java
...
//            Server, ExtensionMapper

public class ArrayOfServer
    implements ADBBean
{
    public static class Factory
    {

        public static ArrayOfServer parse(XMLStreamReader reader)
            throws Exception
        {
            ArrayOfServer object;
            object = new ArrayOfServer();
            ...
            try
            {
                ...
                if(reader.getAttributeValue("http://www.w3.org/2001/XMLSchema-instance", "type") != null)
                {
                    ...
                    if(fullTypeName != null)
                    {
                        ...
                        if(!"ArrayOfServer".equals(type))
                        {
                            String nsUri = reader.getNamespaceContext().getNamespaceURI(nsPrefix);
                            return (ArrayOfServer)ExtensionMapper.getTypeObject(nsUri, type, reader);
                        }
                    }
                }
            }
            ...
            {
                list1.add(Server.Factory.parse(reader));
                loopDone1 = false;
                while(!loopDone1) 
                {
                ...
                object.setItem((Server[])(Server[])ConverterUtil.convertToArray(com/mathworks/internal/dws/client/v3/Server, list1));
            }
            for(; !reader.isStartElement() && !reader.isEndElement(); reader.next());
    ...
    protected void validateItem(Server aserver[])
    {
    }

    public void setItem(Server param[])
    {
        validateItem(param);
        if(param != null)
    ...
    public void addItem(Server param)
    {
        if(localItem == null)
            localItem = new Server[0];
        localItemTracker = true;
        List list = ConverterUtil.toList(localItem);
        list.add(param);
        localItem = (Server[])(Server[])list.toArray(new Server[list.size()]);
    }
        ...
        throws ADBException
    {
        org.apache.axiom.om.OMDataSource dataSource = new ADBDataSource(parentQName, factory) {
            ...
            {
                ArrayOfServer.this.serialize(parentQName, factory, xmlWriter);
            }

            final OMFactory val$factory;
            final ArrayOfServer this$0;

            
            {
                this$0 = ArrayOfServer.this;
                factory = omfactory;
                super(x0, x1);
            }
        }
        ...
        throws XMLStreamException, ADBException
    {
        ...
        if(serializeType)
        {
            ...
            if(namespacePrefix != null && namespacePrefix.trim().length() > 0)
                writeAttribute("xsi", "http://www.w3.org/2001/XMLSchema-instance", "type", (new StringBuilder()).append(namespacePrefix).append(":ArrayOfServer").toString(), xmlWriter);
            else
                writeAttribute("xsi", "http://www.w3.org/2001/XMLSchema-instance", "type", "ArrayOfServer", xmlWriter);
        }
        if(localItemTracker)
            if(localItem != null)
            {

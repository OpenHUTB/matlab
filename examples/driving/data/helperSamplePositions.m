function[PositionMat,varargout]=helperSamplePositions(obj,numActors,varargin)
    ...
    [actorList,s]=addActors(tempScenario,numActors,parsedInputs);
...
function copyRoad(map,currScenario)


    map.RoadHistory=currScenario.RoadHistory;
    map.RoadSegments=currScenario.RoadSegments;
    map.RoadTiles=currScenario.RoadTiles;
    map.RoadCenters=currScenario.RoadCenters;
    map.RoadTileCentroids=currScenario.RoadTileCentroids;
    map.RoadTileMaxRadii=currScenario.RoadTileMaxRadii;
    map.RoadID=currScenario.RoadID;
    map.CachedLaneMarkingVertices=currScenario.CachedLaneMarkingVertices;
    map.CachedLaneMarkingFaces=currScenario.CachedLaneMarkingFaces;
    map.ShowRoadBorders=currScenario.ShowRoadBorders;
    map.IsOpenDRIVERoad=currScenario.IsOpenDRIVERoad;
    map.EndRoadIDsRoadCenters=currScenario.EndRoadIDsRoadCenters;
    map.SampleTime=currScenario.SampleTime;
    map.StopTime=currScenario.StopTime;
    try
        map.MapDimensions=currScenario.MapDimensions;
    catch
    end
    for i=1:numel(currScenario.Actors)
        actor(map,'Position',currScenario.Actors(i).Position);
    end
end

classdef cRoadNetwork
    ...
    methods(Access=public,Hidden=true)
        ...
        function obj=initialiseActor(obj,actors)
            ...
            obj.ActorDetails.ID=zeros(length(actors),1);
            obj.ActorDetails.Position=zeros(length(actors),3);
            obj.ActorDetails.Speed=zeros(length(actors),1);
            ...
            for indx=1:length(actors)
                ...
                if(strcmpi(actorElement.Type,'Car')||...
                    ...
                    obj.ActorDetails.ID(indx)=actorArr(count).ID;
                    obj.ActorDetails.Position(indx,:)=actorPosArr(count,:);
                    obj.ActorDetails.Speed(indx)=0;
        ...
        function obj=update(obj,time)
            ...
            for indx=1:length(obj.Actors)
                ...
                obj.ActorDetails.ID(indx)=actor.ID;
                obj.ActorDetails.Position(indx,:)=[x,y,z];
                obj.ActorDetails.Speed(indx)=actorFuture.MotionModel.ScaledSpeed;
        ...
        function flag=checkCondition(~,actor,baseActor)
            ...
            if((norm(pos-basePos)<=dist||...
                ...
                if(baseActor.RoadType==1&&actor.RoadType==1)

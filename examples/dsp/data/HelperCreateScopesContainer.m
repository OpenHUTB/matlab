function scopesContainer=HelperCreateScopesContainer(ScopesOrModel,varargin)
    ...
    function Scopes=findAllScopes(Model)
        ...
        ap=find_system(Model,'LookUnderMasks','all','BlockType','ArrayPlot');
        sa=find_system(Model,'LookUnderMasks','all','BlockType','SpectrumAnalyzer');
        ts=find_system(Model,'LookUnderMasks','all','BlockType','Scope');
        Scopes={ap{:},ts{:},sa{:}};%#ok<CCAT>
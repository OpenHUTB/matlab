classdef EstimatorTimeSoftmaxLayer<dnnfpga.estimate.EstimatorTransfer

    methods


        function SoftmaxLayerTime=GetLayerTime(this,hPC,cnnp,param,HWparams,InternalArchParam,layerIndex,NumofFcLayers,calData)
            ...
            LayerCompute=this.GetSoftmaxLayerCompTime(hPC,param,cnnp,calData);
            LayerOpt=this.getLayerOpt(LayerInfo,HWparams);
            SoftmaxLayerTime.layer=this.AssembleLayerResult(LayerOpt,LayerCompute,LayerInBurst,LayerOutBurst,InternalArchParam);

        end

        function SoftmaxLayerCompTime=GetSoftmaxLayerCompTime(this,hPC,param,cnnp,calData)
            ...
            DenominatorLatency=ceil((param.matrixSize(1)/cc.threadNumLimit))*cc.SingleSumLatency;


            DenominatorPathLatency=cc.InputOutputLatency+cc.DataMemReadLatency+cc.SingleProdLatency+cc.ExpLatency+cc.RelationalOpLatency+...
            (cc.Int16_To_SingleLatency*log2(cc.threadNumLimit));


            NumeratorLatency=ceil((param.matrixSize(1)/cc.threadNumLimit))*1;
            NumeratorPathLatency=cc.InputOutputLatency+cc.DataMemReadLatency+cc.SingleProdLatency+cc.ExpLatency+cc.RelationalOpLatency+...
            (cc.Int16_To_SingleLatency*log2(cc.threadNumLimit))+...
            cc.DivideLatency;

            TotalPathLatency=NumLayers*(DenominatorLatency+DenominatorPathLatency+...
            NumeratorLatency+NumeratorPathLatency);


            SoftmaxLayerCompTime=(LayerConfigTime+TotalPathLatency);

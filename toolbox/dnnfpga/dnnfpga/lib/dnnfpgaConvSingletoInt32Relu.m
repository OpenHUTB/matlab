...
function dnnfpgaConvSingletoInt32Relu(gcb,kernelDataType,RoundingMode,PipelineLatency)
    ...
    function redrawBusConnect(gcb,curGcbOrig,pos,kernelDataType,RoundingMode,PipelineLatency)
        ...
        if(strcmp(kernelDataType,'single'))
            ...
            h=add_block('hdlsllib/Ports & Subsystems/For Each Subsystem',curGcbOrig,'MakeNameUnique','on','Position',pos,'TreatAsAtomicUnit','off');
        end
        ...
        if(strcmp(kernelDataType,'single'))
            ...
            delete_line(curGcb,'In1/1','Out1/1');
            delete_block([curGcb,'/Out1']);
            delete_block([curGcb,'/In1']);
            set_param([curGcb,'/For Each'],'InputPartition',{'on','on'});
            add_block('dnnfpgaBfpScalinglib/single2int32Relu',[curGcb,'/DTC'],'position',pos);

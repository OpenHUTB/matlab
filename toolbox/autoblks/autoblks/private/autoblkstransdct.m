function[varargout]=autoblkstransdct(varargin)
...
function Initilization(block)
    ...
    case 'Ideal integrated controller'
        SwitchInport(block,'CltchA',0);
        SwitchInport(block,'CltchB',0);
    case 'External control'
        SwitchInport(block,'CltchA',1);
        SwitchInport(block,'CltchB',1);
    end
    ...
    InportNames={'Gear';'CltchA';'CltchB';'EngTrq';'DiffTrq';'Temp'};
    NumberInputPorts(block,InportNames)


    ClutchLocked=get_param(block,'ClutchLocked');
    SynchLocked=get_param(block,'SynchLocked');
    set_param([block,'/Dual Clutch Transmission'],'ClutchLocked',ClutchLocked);
    set_param([block,'/Dual Clutch Transmission'],'SynchLocked',SynchLocked);
...
function IconInfo=DrawCommands(BlkHdl)
    ...
    'CltchA','CltchACmd';'CltchB','CltchBCmd';
    };
    IconInfo=autoblksgetportlabels(BlkHdl,AliasNames);


    IconInfo.ImageName='transmission_dct.png';

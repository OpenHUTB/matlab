function cNet=elabCheckNodeUnitNetwork(this,topNet,blockInfo,dataRate)
    ...
    bc1Type=pir_ufixpt_t(36,0);
    bc2Type=pir_ufixpt_t(2*blockInfo.minWL,0);

    cType=pir_ufixpt_t(30,0);
    ...
    '+satcomhdlsupport','+internal','@DVBS2LDPCDecoder','cgireml','minCalculation.m'),'r');
    ...
    pirelab.getUnitDelayEnabledResettableComp(cNet,betacomp1D,cnu_decomp1,valido,reset,'');
    pirelab.getUnitDelayEnabledResettableComp(cNet,betacomp2D,cnu_decomp2,valido,reset,'');
    pirelab.getUnitDelayComp(cNet,valido,cnu_valid,'');

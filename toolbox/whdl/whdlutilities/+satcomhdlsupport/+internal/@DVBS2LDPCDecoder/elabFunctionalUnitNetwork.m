function fNet=elabFunctionalUnitNetwork(this,topNet,blockInfo,dataRate)
    ...
    'Network',topNet,...
    'Name','FunctionalUnit',...
    'Inportnames',{'data','valid','count','betacomp1','betacomp2','decmpvalid','reset','ddsm'},...
    'InportTypes',[alphaVType,ufix1Type,ufix5Type,bcVType1,bcVType2,ufix1Type,ufix1Type,ufix1Type],...
    ...
    pirelab.instantiateNetwork(fNet,vNet,[data,beta,valid,betavalid,reset,ddsm],...
    ...
    b2Net.addComment(['Beta_Decompress_Unit_',num2str(1)]);
    pirelab.instantiateNetwork(fNet,b2Net,[cnucomp1,cnucomp2,cnuvalid,count,reset],...
    [beta1,betavalid1],['Beta_Decompress_Unit_',num2str(1)]);



    fid=fopen(fullfile(matlabroot,'toolbox','whdl','whdlutilities',...
    '+satcomhdlsupport','+internal','@DVBS2LDPCDecoder','cgireml','addressGeneration.m'),'r');
    ...
    pirelab.getSimpleDualPortRamComp(fNet,[alpha,wraddr,valid_alpha,rdaddr],alphaD,'VariableDelay',blockInfo.memDepth,-1,[],'','',blockInfo.ramAttr_dist);



    aNet=this.elabAposterioriNodeUnitNetwork(fNet,blockInfo,dataRate);
    aNet.addComment('Aposteriori Node Unit');
    pirelab.instantiateNetwork(fNet,aNet,[alphaD,beta1,rdvalidreg],...

function masseyNet=elabMassey(~,topNet,blockInfo,datarate)
    ...
    k=double(blockInfo.MessageLength);
    if(k==239)
        corr=8;
    else
        corr=16;
    ...
    [~,~,~,~,~,tAntiLogTable,tLogTable]=HDLCCSDSRSCodeTables(k);
    ...
    countType=pir_ufixpt_t(nextpow2(2*corr),0);
    count2Type=pir_ufixpt_t(nextpow2(2*corr+1),0);

function tbNet=elabTracebackEngine(~,topNet,blockInfo,dataRate)
    ...
    if(blockInfo.ResetPort)
        inportnames={'tbdata','decdata','minindx','enb','rst'};
        inporttypes=[ramType,ramType,idxType,ufix1Type,ufix1Type];
        inportrates=[dataRate,dataRate,dataRate,dataRate,dataRate];
    else
        inportnames={'tbdata','decdata','minindx','enb'};
        inporttypes=[ramType,ramType,idxType,ufix1Type];
    ...
    tbdata=tbNet.PirInputSignals(1);
    decdata=tbNet.PirInputSignals(2);
    minindx=tbNet.PirInputSignals(3);
    enb=tbNet.PirInputSignals(4);

    decoded=tbNet.PirOutputSignals(1);
    valid=tbNet.PirOutputSignals(2);
    if(blockInfo.ResetPort)
        rst=tbNet.PirInputSignals(5);
    end

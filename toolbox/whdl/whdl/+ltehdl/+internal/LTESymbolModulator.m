classdef(StrictDefaults)LTESymbolModulator<matlab.System
...
%#codegen
    ...
    methods
        ...
        function obj=LTESymbolModulator(varargin)
        ...
        function set.WordLength(obj,val)
            ...
            'scalar','>',2,'<',33},'LTESymbolModulator','Word length');
            obj.WordLength=val;
        end
    end
    ...
    methods(Static,Access=protected)
        function header=getHeaderImpl
            ...
            header=matlab.system.display.Header('ltehdl.internal.LTESymbolModulator',...
            'Title','LTE Symbol Modulator',...
            'Text',text,...
    ...
    methods(Access=protected)
        function icon=getIconImpl(obj,varargin)
            if isempty(obj.inDisp)
                if strcmpi(obj.ModulationSource,'Property')
                    icon=sprintf('LTE Symbol Modulator\nLatency = --');
                else
                    icon=sprintf('LTE Symbol Modulator');
                end
            ...
            if strcmpi(obj.ModulationSource,'Property')
                ...
                if isempty(coder.target)||~coder.internal.isAmbiguousTypes
                    ...
                    if~any((0:4)==(varargin{3}))
                        coder.internal.warning('whdl:LTESymbolModulator:InvalidModSelValue');
                    end
                end

                if isa(varargin{3},'double')||isa(varargin{3},'single')
                    if~ismember(varargin{3},(0:4))
                        modSel=fi(1,0,3,0);
                    else
                ...
                symInd,offAdrs,b0,b1,b2,b3,b4,b5,b6,b7);
            end
        ...
        function[IaddrOut,QaddrOut,validOut]=addrsGenPort(obj,modSel,symInd,...
            offAddrs,b0,b1,b2,b3,b4,b5,b6,b7)

...
function Log10MinusTable=generateLog10MinusTable(cfType)
    ...
    for ii=2:1:numel(tmp)
        tmp1(ii)=bitsrl(tmp1(ii),ii);
    end
    Log10MinusTable=reinterpretcast(tmp1,numerictype(1,cfType.Log2IntermediatePrec+cfType.Log2NumberOfIterations+2,...

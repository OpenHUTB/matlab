...
function[Sign,Exponent,Mantissa]=exp(cfType,aSign,aExponent,aMantissa,denormal,...
    ...
    if(aExponent==cfType.Exponent_Inf_or_NaN)
        [Sign,Exponent,Mantissa]=exp_Inf_or_NaN(cfType,aSign,aExponent,aMantissa);
    else
        [exp_cor,mant_cor]=coder.customfloat.helpers.appendMantissaAndCorrectExponent(cfType,aExponent,aMantissa);
        [Sign,Exponent,Mantissa]=exp_Main(cfType,aSign,exp_cor,mant_cor,denormal,...
        Log2Table,Log2MinusTable,Ln2,Log2_E);
    end
end

function[Sign,Exponent,Mantissa]=exp_Main(cfType,aSign,exp_cor,mant_cor,denormal,...
    Log2Table,Log2MinusTable,Ln2,Log2_E)

    Sign=fi(0,0,1,0);

    [exp_m,mant_m]=exp_Multiply_by_Log2_E(cfType,exp_cor,mant_cor,Log2_E);

    [in_range,mant_ext]=exp_Check_Exp_Range_and_Shift_Mant(cfType,aSign,exp_m,mant_m,denormal);

    if(in_range)
        [exp_out,mant_frac,sticky]=exp_Extract_Exp_Mant(cfType,aSign,mant_ext);


        [y,w]=exp_First_Iteration(mant_frac,Log2Table);


        [y,w]=exp_Main_Iterations(cfType,y,w,Log2Table,Log2MinusTable);


        y=exp_Final_Approximation(cfType,y,w,Ln2);


        [exp_norm,mant_out]=exp_Normalize_Answer(cfType,exp_out,y,denormal);
...
function[exp_norm,mant_out]=exp_Normalize_Answer(cfType,exp_out,y,denormal)
    if(denormal)
        [exp_norm,mant_out]=exp_Normalize_Answer_Denormals(cfType,exp_out,y);
    else
        [exp_norm,mant_out]=exp_Normalize_Answer_Normals(cfType,exp_out,y);
    end
end

function[exp_norm,mant_out]=exp_Normalize_Answer_Denormals(cfType,exp_out,y)
    if(exp_out<=0)
...
function[exp_norm,mant_out]=exp_Normalize_Answer_Normals(cfType,exp_out,y)
    if(exp_out>0)
        exp_norm=fi(exp_out,0,cfType.ExponentLength,0);
        mant_out=bitsliceget(y,cfType.Pow10IntermediatePrec,cfType.Pow10IntermediatePrec-cfType.MantissaLength);
    else
...
function[exp_out,mant_frac,sticky]=exp_Extract_Exp_Mant(cfType,aSign,mant_ext)
...
function[in_range,mant_ext]=exp_Check_Exp_Range_and_Shift_Mant(cfType,aSign,exp_cor,mant_cor,denormal)
...
function[exp_m,mant_m]=exp_Multiply_by_Log2_E(cfType,exp_cor,mant_cor,Log2_E)
...
function[Sign,Exponent,Mantissa]=exp_Inf_or_NaN(cfType,aSign,aExponent,aMantissa)
    Mantissa=aMantissa;

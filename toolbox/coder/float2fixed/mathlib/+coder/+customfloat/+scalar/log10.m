...
function[Sign,Exponent,Mantissa]=log10(cfType,aSign,aExponent,aMantissa,denormal,...
    Log10Table,Log10MinusTable,Log4rt10E,...
    Log10ExpTable)
    ...
    if(aExponent==cfType.Exponent_Inf_or_NaN)||(aSign==1)||...
        ...
        [Sign,Exponent,Mantissa]=log10_Inf_or_NaN(cfType,aSign,aExponent,aMantissa);
    else
        [exp_cor,mant_cor]=coder.customfloat.helpers.appendMantissaAndCorrectExponent(cfType,aExponent,aMantissa);
        [Sign,Exponent,Mantissa]=log10_Main(cfType,exp_cor,mant_cor,denormal,...
        Log10Table,Log10MinusTable,Log4rt10E,...
        Log10ExpTable);
    end
end

function[Sign,Exponent,Mantissa]=log10_Main(cfType,exp_cor,mant_cor,denormal,...
    Log10Table,Log10MinusTable,Log4rt10E,...
    Log10ExpTable)

    [exp_norm,mant_norm]=log10_Normalize_Exp_Mant(cfType,exp_cor,mant_cor,denormal);
    ...
    [L,w,exp_idx,sticky]=log10_Prep(cfType,exp_norm,mant_norm,Log10Table);


    [L,w,exp_idx]=log10_First_Iteration(L,w,exp_idx);
    [L,w]=log10_Second_Iteration(L,w,Log10Table,Log10MinusTable);




    [L,w]=log10_Main_Iterations(cfType,L,w,Log10Table,Log10MinusTable);


    L=log10_Final_Approximation(cfType,L,w,Log4rt10E);


    exp_log10=log10_Extract_ExpLog10(Log10ExpTable,exp_idx);


    Sum=log10_Add_Exp_And_Fractional_Part(cfType,exp_log10,L);


    [Sign,exp_out,mant_out]=log10_Normalize_Sum(cfType,Sum);
...
function[Sign,exp_out,mant_out]=log10_Normalize_Sum(cfType,Sum)
    ...
    if(pos==0)
        ...
        exp_out=fi(0,0,cfType.ExponentLength,0);
        tmp=fi(cfType.ExponentBias-FL-1-2,...
...
function Sum=log10_Add_Exp_And_Fractional_Part(cfType,exp_log10,L)
    ...
    Sum=cast(exp_log10-L,'like',exp_log10);
end


function exp_log10=log10_Extract_ExpLog10(Log10ExpTable,exp_idx)
    exp_log10=Log10ExpTable(exp_idx);
end


function L=log10_Final_Approximation(cfType,L,w,Log4rt10E)
    ...
    if(w~=0)
        ...
        tmp3=fi(tmp_m1*tmp_m2,0,w.FractionLength+6,w.FractionLength+4);
        appx=cast(tmp3*Log4rt10E,'like',L);
        if(sign==1)
            appx(:)=-appx;
        end
...
function[L,w]=log10_Main_Iterations(cfType,L,w,Log10Table,Log10MinusTable)
    for ii=coder.unroll(3:1:cfType.Log2NumberOfIterations)
        [L,w]=log10_Iteration(L,w,ii,Log10Table,Log10MinusTable);
    end
end

function[L,w]=log10_Iteration(L,w,ii,Log10Table,Log10MinusTable)
...
function[L,w,exp_idx,sticky]=log10_Prep(cfType,exp_norm,mant_norm,Log10Table)
    ...
    L=cast(0,'like',Log10Table(1));
    sticky=tmp;
end

function[L,w,exp_idx]=log10_First_Iteration(L,w,exp_idx)
    if(w>1)
...
function[L,w]=log10_Second_Iteration(L,w,Log10Table,Log10MinusTable)
...
function[exp_norm,mant_norm]=log10_Normalize_Exp_Mant_Normal(cfType,exp_a,mant_a)
...
function[exp_norm,mant_norm]=log10_Normalize_Exp_Mant_Denormal(cfType,exp_a,mant_a)
...
function[exp_norm,mant_norm]=log10_Normalize_Exp_Mant(cfType,exp_cor,mant_cor,denormal)
    if(denormal)
        [exp_norm,mant_norm]=log10_Normalize_Exp_Mant_Denormal(cfType,exp_cor,mant_cor);
    else
        [exp_norm,mant_norm]=log10_Normalize_Exp_Mant_Normal(cfType,exp_cor,mant_cor);
    end
end

function[Sign,Exponent,Mantissa]=log10_Inf_or_NaN(cfType,aSign,aExponent,aMantissa)

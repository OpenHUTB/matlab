...
classdef HDLAcos<coder.internal.mathfcngenerator.HDLLookupTable
    methods

        function obj=HDLAcos(varargin)
            obj=obj@coder.internal.mathfcngenerator.HDLLookupTable(varargin{:});
            obj.CandidateFunction=@(x)acos(x);
            if(nargin<1)
    ...
    methods(Access=public)
        ...
        function[ValidBool,ErrorStr]=InputRangeValidate(obj)
            ...
            if(strcmpi(obj.Mode,'ShiftAndAdd'))
                ...
                if(~ValidBool)
                    ErrorStr=message('float2fixed:MFG:Acos_CORDIC_Err').getString();
                end
            else

                ValidBool=(obj.InputExtents(1)>=-1)&&(obj.InputExtents(2)<=1);
                if(~ValidBool)
                    ErrorStr=message('float2fixed:MFG:Acos_Err').getString();
                end
            end
            return;
        end

...
classdef HDLCos<coder.internal.mathfcngenerator.HDLPeriodic
    methods

        function obj=HDLCos(varargin)
            obj=obj@coder.internal.mathfcngenerator.HDLPeriodic(varargin{:});
            obj.Period=2*pi;
            obj.CandidateFunction=@(x)cos(x);
            if(nargin<1)
    ...
    methods(Access=public)
        ...
        function[ValidBool,ErrorStr]=InputRangeValidate(obj)
            ...
            if(strcmpi(obj.Mode,'ShiftAndAdd'))
                ...
                if(~ValidBool)
                    ErrorStr=message('float2fixed:MFG:Cos_Err').getString();
                end
            end
        end
    end

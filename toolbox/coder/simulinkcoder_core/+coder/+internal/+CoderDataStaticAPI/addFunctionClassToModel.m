function addFunctionClassToModel(sourceDD,varargin)
    ...
    validProperties={'Name','Description','FunctionName','MemorySection'};
    for idx=1:2:length(varargin)
        if~any(ismember(validProperties,varargin{idx}))
            error([varargin{idx},' is an invalid property name. ',...
            'Valid names are Name, Description, FunctionName, MemorySection']);
        end
    end
    ...
    ms=hlp.createEntry(dd,'FunctionClass',msName);

function dlgStruct=getDialogSchema(this,dummy)
    ...
    outerRadius=rfblksGetLeafWidgetBase('edit','','OuterRadius',...
    this,'OuterRadius');
    outerRadius.RowSpan=[1,1];
    outerRadius.ColSpan=[lwidget,rwidget];

    outerRadiusprompt=rfblksGetLeafWidgetBase('text','Outer radius (m):',...
    'OuterRadiusPrompt',0);
    outerRadiusprompt.RowSpan=[1,1];
    outerRadiusprompt.ColSpan=[lprompt,rprompt];


    innerRadius=rfblksGetLeafWidgetBase('edit','','InnerRadius',...
    this,'InnerRadius');
    innerRadius.RowSpan=[2,2];
    innerRadius.ColSpan=[lwidget,rwidget];

    innerRadiusprompt=rfblksGetLeafWidgetBase('text','Inner radius (m):',...
    'InnerRadiusPrompt',0);
    innerRadiusprompt.RowSpan=[2,2];
    innerRadiusprompt.ColSpan=[lprompt,rprompt];
    ...
    mainPane.Items={outerRadius,innerRadius,muR,epsilonR,lossTangent,sigmaCond,...
    lineLength,stubMode,termination,outerRadiusprompt,innerRadiusprompt,...
    muRprompt,epsilonRprompt,lossTangentprompt,sigmaCondprompt,...
    lineLengthprompt,stubModeprompt,terminationprompt,...
    spacerMain};

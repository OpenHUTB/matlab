classdef MatchedFilterPlot<phased.apps.internal.WaveformViewer.PlotData
...
Layout
    ...
    methods
        function self=MatchedFilterPlot(parent)
            self.View=parent;
            self.Figure=self.View.MatchedFilterCoefficientsFig;
            ...
            self.TopAxes=axes('Parent',self.Panel,'Position',[0.1,0.57,0.8,0.35]);
            self.BottomAxes=axes('Parent',self.Panel,'Position',[0.1,0.11,0.8,0.35]);

            linkaxes([self.TopAxes,self.BottomAxes],'x')
            linkaxes([self.View.RealAndImaginary.TopAxes,self.View.RealAndImaginary.BottomAxes,self.TopAxes,self.BottomAxes],'x')
        end
        function matchedfilter(self,data)
            self.updatePlot(data);
            x=self.Wav;
            y=self.Compression(x);
            l=(0:length(y)-1)/self.SampleRate;
            ...
            plot(self.TopAxes,x,abs(y));
            phased.apps.internal.WaveformViewer.SetLimits(abs(y),self.TopAxes)
            set(self.TopAxes,'Tag','MatchMagAxes');
            if k>1
            ...
            plot(self.BottomAxes,x,angle(y));
            phased.apps.internal.WaveformViewer.SetLimits(angle(y),self.BottomAxes)
            set(self.BottomAxes,'Tag','MatchPhaseAxes');
            if k>1
        ...
        function genCode(self,strWriter)
            ...
            addcr(strWriter,['l = (0:length(y)-1)/Fs;',newline...
            ,'subplot(2,1,1);',newline...
            ,'[x, scale, ~] = engunits(l);',newline...
            ,newline,'plot(x,abs(y));'...
            ...
            ,'subplot(2,1,2);',newline,'plot(x,angle(y));'...

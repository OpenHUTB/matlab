classdef UpdateButton<slrealtime.internal.SLRTComponent




    properties(Access=protected)
        tgEventsTriggeringUpdateGUI=...
        {...
        }
    end

    methods(Access=public)
        function delete(this)
            this.clearListeners();
        end
    end

    properties(Access=public)
        UpdateIcon='slrtUpdateIcon.png';
        UpdateText=message('slrealtime:appdesigner:UpdateSoftwareText').getString();



FontName
FontSize
FontWeight
FontAngle
FontColor
IconAlignment
HorizontalAlignment
VerticalAlignment
    end

    properties(Access={?slrealtime.ui.container.Menu,?slrealtime.internal.SLRTComponent},Transient,NonCopyable)
        Button matlab.ui.control.Button
    end

    properties(Access={?slrealtime.internal.SLRTComponent},Transient,NonCopyable)
        Grid matlab.ui.container.GridLayout
    end

    properties(Access={?slrealtime.internal.SLRTComponent},Transient,NonCopyable,SetObservable)




Completed
    end

    methods(Access=protected)
        function setup(this)


            buttonWidth=65;
            buttonHeight=65;



            this.Grid=uigridlayout(this,[1,1],...
            'Padding',0,'RowSpacing',0,'ColumnSpacing',0);
            this.Grid.ColumnWidth={'1x'};
            this.Grid.RowHeight={'1x'};



            this.Button=uibutton(this.Grid,...
            'ButtonPushedFcn',@(o,e)this.buttonPushed());
            this.Button.Layout.Row=1;
            this.Button.Layout.Column=1;
            this.Button.HorizontalAlignment='center';
            this.Button.IconAlignment='top';



            this.Position=[100,100,buttonWidth,buttonHeight];
            this.FontName=this.Button.FontName;
            this.FontSize=this.Button.FontSize;
            this.FontWeight=this.Button.FontWeight;
            this.FontAngle=this.Button.FontAngle;
            this.FontColor=this.Button.FontColor;
            this.IconAlignment=this.Button.IconAlignment;
            this.HorizontalAlignment=this.Button.HorizontalAlignment;
            this.VerticalAlignment=this.Button.VerticalAlignment;
            this.BackgroundColor=this.Button.BackgroundColor;
        end

        function update(this)
            if this.firstUpdate
                this.firstUpdate=false;



                if isempty(this.GetTargetNameFcnH)
                    this.initTarget([]);
                end
            end

            this.Button.FontName=this.FontName;
            this.Button.FontSize=this.FontSize;
            this.Button.FontWeight=this.FontWeight;
            this.Button.FontAngle=this.FontAngle;
            this.Button.FontColor=this.FontColor;
            this.Button.IconAlignment=this.IconAlignment;
            this.Button.HorizontalAlignment=this.HorizontalAlignment;
            this.Button.VerticalAlignment=this.VerticalAlignment;
            this.Button.BackgroundColor=this.BackgroundColor;

            if this.isDesignTime()

                this.Button.Enable='on';
                this.Button.Icon=this.UpdateIcon;
                this.Button.Text=this.UpdateText;
                this.Button.Tooltip='';
            else

                this.updateGUI([]);
            end
        end
    end

    methods
        function set.UpdateIcon(this,value)
            slrealtime.internal.SLRTComponent.validateImageFile('UpdateIcon',value);
            this.UpdateIcon=value;
        end
    end

    methods(Access=public,Hidden)
        function disableControlForInvalidTarget(this)
            this.Button.Enable='off';
            this.Button.Icon=this.UpdateIcon;
            this.Button.Text=this.UpdateText;
            this.Button.Tooltip=message('slrealtime:appdesigner:InvalidTargetTooltip',...
            this.GetTargetNameFcnH()).getString();
        end

        function updateGUI(this,~)
            tg=this.tgGetTargetObject();
            if isempty(tg),return;end

            if this.isSimulinkNormalMode()


                this.Button.Enable='off';
                this.Button.Icon=this.UpdateIcon;
                this.Button.Text=this.UpdateText;
                this.Button.Tooltip='';
            else
                this.Button.Enable='on';
                this.Button.Icon=this.UpdateIcon;
                this.Button.Text=this.UpdateText;
                this.Button.Tooltip=message('slrealtime:appdesigner:UpdateSoftwareTooltip',...
                this.GetTargetNameFcnH()).getString();
            end
        end
    end

    properties(Access=private)
UpdateCompletedListener
UpdateFailedListener
RebootIssuedListener

RebootIssued
    end

    methods(Access=private)
        function clearListeners(this)
            delete(this.UpdateCompletedListener);
            this.UpdateCompletedListener=[];
            delete(this.UpdateFailedListener);
            this.UpdateFailedListener=[];
            delete(this.RebootIssuedListener);
            this.RebootIssuedListener=[];
        end

        function updateCompleted(this)
            this.clearListeners;
            this.closeProgressDlg();

            c=onCleanup(@()this.set('Completed',true));

            if this.RebootIssued

                tg=this.tgGetTargetObject();
                if isempty(tg),return;end

                msg=message('slrealtime:appdesigner:UpdateSoftwareRebooting',this.GetTargetNameFcnH());
                title=message('slrealtime:appdesigner:UpdateSoftwareConfigure');
                this.openProgressDlg(msg.getString(),title.getString());
                startTime=tic;
                pause(1);
                timeoutVal=240;
                timeout=true;
                while toc(startTime)<timeoutVal
                    if ispc
                        cmd=['ping -n 1 ',tg.TargetSettings.address];
                    else
                        cmd=['ping -c 1 ',tg.TargetSettings.address];
                    end
                    [status,result]=system(cmd);
                    if~status&&contains(result,'TTL','IgnoreCase',true)
                        timeout=false;
                        break;
                    end
                    pause(2);
                end
                this.closeProgressDlg();

                if timeout
                    msg=message('slrealtime:appdesigner:UpdateSoftwareRebootTimeout',this.GetTargetNameFcnH());
                    title=message('slrealtime:appdesigner:UpdateSoftwareConfigure');
                    uialert(...
                    ancestor(this.Parent,'figure'),...
                    msg.getString(),title.getString());
                    return;
                end
            end

            msg=message('slrealtime:appdesigner:UpdateSoftwareCompleted',this.GetTargetNameFcnH());
            title=message('slrealtime:appdesigner:UpdateSoftwareConfigure');
            uiconfirm(ancestor(this.Parent,'figure'),...
            msg.getString(),title.getString(),'Icon','success',...
            'Options',{getString(message('MATLAB:uitools:uidialogs:OK'))});
        end

        function updateFailed(this)
            this.clearListeners;
            this.closeProgressDlg();




            this.Completed=true;
        end

        function closeConfirmCB(this,tg,e)
            if e.SelectedOptionIndex==1
                msg=message('slrealtime:appdesigner:UpdateSoftwareConfiguring',this.GetTargetNameFcnH());
                title=message('slrealtime:appdesigner:UpdateSoftwareConfigure');
                this.openProgressDlg(msg.getString(),title.getString());

                this.RebootIssued=false;
                this.UpdateCompletedListener=addlistener(tg,'UpdateCompleted',@(src,evnt)updateCompleted(this));
                this.UpdateFailedListener=addlistener(tg,'UpdateFailed',@(src,evnt)updateFailed(this));
                this.RebootIssuedListener=addlistener(tg,'RebootIssued',@(src,evnt)set(this,'RebootIssued',true));
                try
                    tg.update();
                catch ME
                    this.clearListeners;
                    this.closeProgressDlg();
                    this.uialert(ME);
                    this.Completed=true;
                    return;
                end
            else
                this.Completed=true;
            end
        end
    end

    methods(Access={?slrealtime.ui.container.Menu,?slrealtime.internal.SLRTComponent})
        function buttonPushed(this)
            tg=this.tgGetTargetObject();
            if isempty(tg),return;end

            this.Completed=false;

            confirmMsg=message('slrealtime:appdesigner:UpdateSoftwareConfirm',this.GetTargetNameFcnH());
            confirmTitle=message('slrealtime:appdesigner:UpdateSoftwareConfigure');
            uiconfirm(...
            ancestor(this.Parent,'figure'),...
            confirmMsg.getString(),...
            confirmTitle.getString(),...
            'CloseFcn',@(o,e)closeConfirmCB(this,tg,e));
        end
    end
end

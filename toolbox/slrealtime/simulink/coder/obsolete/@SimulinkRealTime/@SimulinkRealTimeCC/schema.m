function schema
    ...
    h=schema.class(hCreateInPackage,'SimulinkRealTimeCC',hDeriveFromClass);
    ...
    p=add_prop(h,'RTWCAPISignals','on/off');
    p.FactoryValue='off';

    p=add_prop(h,'RTWCAPIParams','on/off');
    p.FactoryValue='on';

    p=add_prop(h,'RTWCAPIStates','on/off');
    p.FactoryValue='off';

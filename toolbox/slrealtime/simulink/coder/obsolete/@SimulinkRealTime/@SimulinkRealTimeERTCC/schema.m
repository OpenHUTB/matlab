function schema
    ...
    hCreateInPackage=findpackage('SimulinkRealTime');
    ...
    h=schema.class(hCreateInPackage,'SimulinkRealTimeERTCC',hDeriveFromClass);




    p=add_prop(h,'DataDefinitionFile','string');
    p.FactoryValue='';

    p=add_prop(h,'DataReferenceFile','string');
    p.FactoryValue='';


    if isempty(findtype('CONFIG_TARGET_DATADEFINITION_ENUM'))
        schema.EnumType('CONFIG_TARGET_DATADEFINITION_ENUM',...
        {'Auto','InSourceFile','InSeparateSourceFile'},[0,1,2]);
    end
    p=add_prop(h,'GlobalDataDefinition',...
    'CONFIG_TARGET_DATADEFINITION_ENUM');
    p.FactoryValue='Auto';

    if isempty(findtype('CONFIG_TARGET_DATAREFERENCE_ENUM'))
        schema.EnumType('CONFIG_TARGET_DATAREFERENCE_ENUM',...
        {'Auto','InSourceFile','InSeparateHeaderFile'},[0,1,2]);
    end

    p=add_prop(h,'GlobalDataReference','CONFIG_TARGET_DATAREFERENCE_ENUM');
    p.FactoryValue='Auto';


    if isempty(findtype('CONFIG_TARGET_MODULENAMING_ENUM'))
        schema.EnumType('CONFIG_TARGET_MODULENAMING_ENUM',...
        {'Unspecified','SameAsModel','UserSpecified'},[0,1,2]);
    end
    p=add_prop(h,'ModuleNamingRule','CONFIG_TARGET_MODULENAMING_ENUM');
    p.FactoryValue='Unspecified';
    p.SetFunction=@setModuleNamingRule;
    p.AccessFlags.Serialize='off';

    p=add_prop(h,'ModuleName','string');
    p.TargetCCPropertyAttributes.set_prop_attrib('GRANDFATHERED');
    p.FactoryValue='';
    p.AccessFlags.Serialize='off';

    p=add_prop(h,'GenerateASAP2','slbool');
    p.FactoryValue='off';

    p=add_prop(h,'ERTSrcFileBannerTemplate','string');
    p.FactoryValue='ert_code_template.cgt';

    p=add_prop(h,'ERTHdrFileBannerTemplate','string');
    p.FactoryValue='ert_code_template.cgt';

    p=add_prop(h,'ERTDataSrcFileTemplate','string');
    p.FactoryValue='ert_code_template.cgt';

    p=add_prop(h,'ERTDataHdrFileTemplate','string');
    p.FactoryValue='ert_code_template.cgt';

    p=add_prop(h,'ERTCustomFileTemplate','string');
    p.FactoryValue='example_file_process.tlc';

    p=add_prop(h,'SignalDisplayLevel','slint');
    p.FactoryValue=10;

    p=add_prop(h,'ParamTuneLevel','slint');
    p.FactoryValue=10;

    if isempty(findtype('CONFIG_TARGET_INITIALVALUESOURCE_ENUM'))
        schema.EnumType('CONFIG_TARGET_INITIALVALUESOURCE_ENUM',...
        {'Model','DataObject'},[0,1]);
    end
    p=add_prop(h,'InitialValueSource',...
    'CONFIG_TARGET_INITIALVALUESOURCE_ENUM');
    p.TargetCCPropertyAttributes.set_prop_attrib('GRANDFATHERED');
    p.FactoryValue='Model';


    p=add_prop(h,'RTWCAPISignals','slbool');
    p.FactoryValue='off';

    p=add_prop(h,'RTWCAPIParams','slbool');
    p.FactoryValue='on';

    p=add_prop(h,'RTWCAPIStates','slbool');
    p.FactoryValue='off';
    ...
    p=add_prop(h,'UseERAMForLogging','on/off');
    p.FactoryValue='off';
    ...
    m=schema.method(h,'setModuleNamingRule');
    s=m.Signature;
    s.varargin='off';
    s.InputTypes={'handle','CONFIG_TARGET_MODULENAMING_ENUM'};
    s.OutputTypes={'CONFIG_TARGET_MODULENAMING_ENUM'};

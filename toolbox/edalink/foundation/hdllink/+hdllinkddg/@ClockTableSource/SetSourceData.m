function SetSourceData(this,srcData,rowSelect)








    this.RowSources=hdllinkddg.ClockRowSource('/top/clk1','Rising','2');

    this.NumRows=size(srcData,1);
    this.NumCols=size(srcData,2);

    for idx=1:this.NumRows
        srcData{idx,2}=this.UddUtil.EnumInt2Str('CoSimClockTypeEnum',srcData{idx,2});


        this.RowSources(idx)=hdllinkddg.ClockRowSource(srcData{idx,:});
    end

    if(this.NumRows>=rowSelect),this.CurrRow=rowSelect;
    elseif(this.NumRows>0),this.CurrRow=1;
    else,this.CurrRow=0;
    end

end

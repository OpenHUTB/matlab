function Autotimescale(this,isPushButton)
    ...
    currentScaleFactor=l_GetCurrentScaleFactorAsNum(this);

    allSampleTimes=portSampleTimes;
    ...
    if(length(allSampleTimes)==1)
        ...
        okidx=allSampleTimes~=0;
        baseTime=computeBaseRate(allSampleTimes(okidx));
    end
    ...
    currentScaleFactorIsValid=isSecondMultipleOfFirst(hdlPrecision,baseTime*currentScaleFactor);
...
...
    ...
    h=hdllinkddg.AutoTimescaleDlg(this,newScaleFactor,portSampleTimes,hdlPrecision,isPushButton,currentScaleFactor,currentScaleFactorIsValid);
    DAStudio.Dialog(h);
...
function[clkSampleTimes,rstSampleTimes]=l_GetClockResetSampleTimes(this)
    clkRstData=this.ClockResetTableSource.GetSourceData;
    numClkRsts=size(clkRstData,1);
    clkSampleTimes=[];clkIdx=0;
    rstSampleTimes=[];rstIdx=0;
    for idx=1:numClkRsts
        if(clkRstData{idx,2}==1||clkRstData{idx,2}==2)
            clkIdx=clkIdx+1;


            clkSampleTimes(clkIdx)=slResolve(clkRstData{idx,3},this.Block.Handle)/2;%#ok<AGROW> 
        else
            rstIdx=rstIdx+1;
            rstSampleTimes(rstIdx)=slResolve(clkRstData{idx,3},this.Block.Handle);%#ok<AGROW> 
        end
    end
end
...
function hdlPrecision=l_GetHdlPrecision(this)
    ...
    hdlPrecision=10^(this.UserData.TimePrecision);
end

function sf=l_GetCurrentScaleFactorAsNum(this)
    if strcmpi(this.TimingMode,'Tick')
        sfu=l_GetHdlPrecision(this);
    else
        sfu=10^(CosimWizardPkg.CosimWizardData.precStrToExp(['1',this.TimingMode]));
    end


    sfm=slResolve(this.TimingScaleFactor,this.Block.Handle);
    sf=sfm*sfu;
end

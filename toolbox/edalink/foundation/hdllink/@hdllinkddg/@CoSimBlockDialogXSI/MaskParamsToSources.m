function MaskParamsToSources(this)
    ...
    this.PortExtendedTableSource=hdllinkddg.PortExtendedTableSource('PortExtendedTable',portRows);

    clockRows=l_ClockMaskParamsToRows(this);
    this.ClockResetTableSource=hdllinkddg.ClockResetTableSource('ClockResetTable',clockRows);
...
function portRows=l_PortMaskParamsToRows(this)
    ...
    xsiData=this.Block.UserData;
    PortHDLTypes=num2cell([xsiData.HdlSigInfo.Type])';
    PortHDLDims=cellfun(@(x)(mat2str(x)),{xsiData.HdlSigInfo.Dims},'UniformOutput',false)';
    ...
    portRows=[PortPaths,PortModes,PortHDLTypes,PortHDLDims,PortTimes,PortDataTypes,PortSigns,PortFracs];

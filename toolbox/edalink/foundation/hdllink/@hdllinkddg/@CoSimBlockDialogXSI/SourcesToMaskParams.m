...
function l_PortRowsToMaskParams(this)
    ...
    srcData=this.PortExtendedTableSource.GetSourceData;
    colPos=this.PortExtendedTableSource.colPos;
    PortTypeEnum=l_PortDataTypeCoversion(srcData(:,colPos.datatype+1),srcData(:,colPos.sign+1));

    PortPaths=sprintf('%s;',srcData{:,colPos.path+1});
    PortPaths(end)=[];
    PortModes=['[',sprintf('%d ',srcData{:,colPos.ioMode+1}),']'];
    PortTimes=['[',sprintf('%s,',srcData{:,colPos.sampleTime+1})];

    PortTimes(end)=']';
    PortTypeEnum=['[',sprintf('%d ',PortTypeEnum{:}),']'];
    PortFracLengths=['[',sprintf('%s,',srcData{:,colPos.fracLength+1})];
    PortFracLengths(end)=']';
...
function l_ClockRowsToMaskParams(this)
    ...
    srcData=this.ClockResetTableSource.GetSourceData;

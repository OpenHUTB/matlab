classdef(Sealed)NaNEvaluationInSampleData<FunctionApproximation.internal.problemvalidator.ProblemDefinitionValidator
    ...
    properties
        ErrorID='SimulinkFixedPoint:functionApproximation:functionEvaluatesToNaNAt'
Coordinates
    end

    methods
        function isValid=validate(this,problemDefinition)
            indices=find(isnan(problemDefinition.SampledTableData(:)));
    ...
    methods
        function diagnostic=getDiagnostic(this,problemDefinition)
            if size(this.Coordinates,1)>5
                diagnostic=MException(message('SimulinkFixedPoint:functionApproximation:functionEvaluatesToNaNAtAndMore',...

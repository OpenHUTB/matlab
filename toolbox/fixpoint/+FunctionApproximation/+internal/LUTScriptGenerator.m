classdef LUTScriptGenerator<FunctionApproximation.internal.LUTScriptFactory
    ...
    methods
        function this=LUTScriptGenerator(tableData,inputType,outputType,spacing)
        ...
        function codeString=getMATLABScript(this)
            codeString=FunctionApproximation.internal.getBasicTemplateString(this.InterpMethod,this.Spacing);
            ...
            assertInputString=getAssertInputString(this);
            codeString=strrep(codeString,'#ASSERT_INPUT',assertInputString);
            ...
            prelookupString=getpreloookupString(this);
        ...
        function assertInputString=getAssertInputString(this)
            assertInputString=FunctionApproximation.internal.getAssertInputString(numel((this.DataTypes.InputType)));
        end
        ...
        function indexSearchString=getIndexSearchString(this)
            indexSearchString=FunctionApproximation.internal.getIndexSearchString(this.Spacing,this.DataTypes.InputType);
        end
        ...
        function interpLogicStr=getStringsForInterpolation(this)
            ...
            strategy=FunctionApproximation.internal.interpolationstrategy.InterpolationStrategyFactory.getInterpolationStrategy(this.InterpMethod);
        ...
        function prelookupString=getpreloookupString(this)
            strategy=FunctionApproximation.internal.prelookupfunction.PrelookupStrategyFactory.getPrelookupStrategy(this.InterpMethod);
        ...
        function context=getInterpolationStrategyContext(this)
            context=FunctionApproximation.internal.interpolationstrategy.InterpolationStrategyContext;
            context.OutputType=this.DataTypes.OutputType;
            context.TableValuesType=this.DataTypes.TableValuesType;
            context.NumberOfInputs=numel(this.DataTypes.InputType);
        end

        function codeString=getCommentstoAddString(~,codeString,functionName,commentsToAdd)
            [headerString,probDefString]=FunctionApproximation.internal.getCommentStrings(functionName,commentsToAdd);

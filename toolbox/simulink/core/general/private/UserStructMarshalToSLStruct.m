...
function body = UserStructMarshalToSLStruct( busName, width, isNd, widthStr, dstAddr, src, offsetList, level, isCplx, busVisitedMap, model, SFunMajorityIdx, DataTypeNames, portNumber, isParentArray, currBusPortInfoIdx )
...
src = sprintf( [ '(( %s *)%s)[%s]' ], dtname, src, loopCounterVar );
...
if strcmp( widthStr, 'topLevel' ) && isequal( level, 0 )
forLoopBegin = sprintf( [ '\n%s{\n%s%s\n%s%s \n%sconst int_T *dstdims%d = ssGetOutputPortDimensions(S, %d); \n%sconst int_T numdims%d = 2; \n%sfor (%s = 0; %s < busInfo[%d].numElems; %s++) {' ],  ...
...
forLoopBegin = sprintf( [ '%s\n%s%s = linear_idx(dstdims%d, numdims%d, %s);' ], forLoopBegin, tabSupplier( level + 2 ), rowMajorLoopCounterVar, level, level, loopCounterVar );
else 
if isParentArray
level = level + 1;
end 
...
inCurrForLoopNew = UserStructMarshalToSLStruct( eDType, eWidth, isNd, eWidthStr, dstAddr, eSrc, eOffsetList, level, eIsCplx, busVisitedMap, model, SFunMajorityIdx, DataTypeNames, portNumber, isParentArrayToSend,  - 1 );
...
if ~isempty( eOffsetStr )
eDst = sprintf( [ '%s + %s' ], dstAddr, eOffsetStr );
else 
eDst = sprintf( [ '%s' ], dstAddr );
end 
...
inCurrForLoop = sprintf( [ '%s\n%s*((%s*)(%s)) = %s;' ],  ...
inCurrForLoop, tabSupplier( l ), eDType, eDst, eSrc );
...
inCurrForLoop, tabSupplier( l ), eDst, optAddr, eSrc, eWidth, eSize );
else 
inCurrForLoop = sprintf( [ '%s\n%s/*Copy from S-Function to Simulink for %s in %s bus to account for row-major to column-major*/' ], inCurrForLoop, tabSupplier( l ), eName, busName );
if eIsCplx
inCurrForLoop = sprintf( [ '%s\n%sNDTransposeByDstSpecs((void*)(%s), (const void*)(%s%s),%s, %s, 2*%s);' ],  ...
inCurrForLoop, tabSupplier( l ), eDst, optAddr, eSrc, eDims, eNumDims, eSize );
else 
inCurrForLoop = sprintf( [ '%s\n%sNDTransposeByDstSpecs((void*)(%s), (const void*)(%s%s),%s, %s, %s);' ],  ...
inCurrForLoop, tabSupplier( l ), eDst, optAddr, eSrc, eDims, eNumDims, eSize );
end 
...
% Decoded using De-pcode utility v1.2 from file /tmp/tmpVCjXQh.p.

...
classdef HierarchySpreadsheetRow<handle
...
m_Children
    ...
    methods
        function obj=HierarchySpreadsheetRow(aDlgSource,aSource,aIsSlimDialog,aRealPath,aName,aDefaultValue,aDefaultValueInstSpec,aDefaultValueInternal,aModelName,aValue,aInstanceSpecific,aSIDPath,aIsFromProtectedModel,aParameterCreatedFrom)
            try
                if nargin==4
            ...
            try
                ...
                case{this.instanceSpecificColumn}
                    if(this.m_DlgSource.isHierarchySimulating)
                        bIsReadOnly=mdlrefddg_cb('EnableParamArgValues',this.m_DlgSource);
        ...
        function setInstanceSpecificPropValue(this,aPropValue)
            ...
            if this.getIsArgument
                ...
                for i=1:length(aDlgs)
                    dlg=aDlgs(i);
                    src=dlg.getWidgetSource('ModelRefArgumentsDDGTreeSpreadsheet');
                    src.showPromotedModelArgumentDialog();
                end
            end

            if this.m_SlimDialog
                mdlrefddg_cb('SetInstSpecModelArgs',this.m_DlgSource);
            else
                aDlgs=DAStudio.ToolRoot.getOpenDialogs(this.m_DlgSource);
                for i=1:length(aDlgs)
                    aDlg=aDlgs(i);
                        ...
                        if(this.m_IsTerminalNode)
                            if isequal(this.m_Value,this.m_DefaultValueInternal)
                                ...
                                try
                                    ...
                                    cachedInfo=slInternal('getInstanceParameterCachedInfo',...
                                    [blk.Path,'/',blk.Name],this.m_SIDPath);
                                    if~isempty(cachedInfo)&&...
                                        isa(cachedInfo.CompiledValue,...
                                        "Simulink.LookupTable")&&...
                                        isequal(cachedInfo.CompiledValue.BreakpointsSpecification,...
                                        "Reference")&&...
                                        ~cachedInfo.CompiledValue.IsSynthesizedInstanceParameterValue
                                        aStyle.Tooltip=DAStudio.message('Simulink:dialog:ModelRefArgsTableInstanceSpecificComAxisLUTValueTooltip',this.m_Value);
                                        aStyle.Icon='toolbox/shared/dastudio/resources/info.png';
                                        aStyle.IconAlignment='right';
                                    end
                                catch
                                end
                            end
        ...
        function rtn=hasPropertyActions(this,propertyName)%#ok<INUSL>
            ...
            rtn=false;
            if slfeature('VarNavEditTimeChecksModelArguments')>0&&strcmp(propertyName,'Value')
                rtn=true;
            end
        end
        ...
        function rtn=getPropertyActions(this,propertyName,propertyValue)
            ...
            if slfeature('VarNavEditTimeChecksModelArguments')>0&&strcmp(propertyName,'Value')
                ...
                try
                    ...
                    rtn=Simulink.ModelReference.internal.PropActionsBuilder.buildAndAppend(...
                    blkFullName,this.m_SIDPath,internalValue,true,otherActions);
                catch
                    rtn=otherActions;
                end
            end
                ...
                if(~isempty(this.m_RealPath))
                    for i=1:this.m_RealPath.getLength()
                        ...
                        if(~isempty(displayString))
                            displayString=[displayString,':'];%#ok<AGROW>
                        end
                        displayString=[displayString,lastBlock];%#ok<AGROW>
                    end
                end
            end

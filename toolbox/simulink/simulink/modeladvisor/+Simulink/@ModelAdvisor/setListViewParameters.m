function success=setListViewParameters(this,varargin)
    ...
    if nargin==2
        if~isempty(this.ActiveCheck)
            this.ActiveCheck.setListViewParameters(varargin{1});
            success=true;
        end
    elseif nargin==3
        for i=1:length(this.CheckCellArray)
            if strcmp(this.CheckCellArray{i}.ID,varargin{1})
                this.CheckCellArray{i}.setListViewParameters(varargin{2});
                success=true;
                return
            end
        end
        ...
        DAStudio.error('Simulink:tools:MAIncorrectAPIUsage','setListViewParameters');
    end

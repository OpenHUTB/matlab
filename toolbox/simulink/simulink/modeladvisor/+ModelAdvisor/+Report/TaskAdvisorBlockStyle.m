classdef TaskAdvisorBlockStyle<ModelAdvisor.Report.TaskAdvisorStyleFactory
    methods
        function obj=TaskAdvisorBlockStyle
            obj.Name=[getString(message('ModelAdvisor:engine:GroupBy')),' ',getString(message('ModelAdvisor:engine:Block'))];
        end
        ...
        function html=generateReport(this,TaskObj)
            if isa(TaskObj,'ModelAdvisor.Group')
                ...
                fts=ModelAdvisor.Report.Utils.sortObjs(elementsObjs,'Block',true);
                html=fts;
            else

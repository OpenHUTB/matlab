function domObjs=emitDOMforChecks(obj,report)




    domObjs=loc_emitDOMforChecks(report,obj.CheckList,obj.ModelName,0);

end

function[resultHTML,dot_counter]=loc_emitDOMforChecks(rptObj,Checks,ModelName,dot_counter)
    resultHTML={};
    ...
    for i=1:length(Checks)
        ...
        checkObj=maObj.getCheckObj(Checks{i});
        subRresultHTML=emitDOMforLeafNode(rptObj,checkObj,maObj);
        resultHTML={resultHTML{:},subRresultHTML{:},DOM_insert_newline};
    end
end
...
function resultHTML=emitDOMforLeafNode(rptObj,this,maObj)
    ...
    while~strcmp(dp.CurrentHoleId,'#end#')
        ...
        case 'CheckTitle'
            ...
            CheckTitle=mlreportgen.dom.Heading(1);
            CheckTitle.WhiteSpace='preserve';
            imageIcon=DOM_Get_icon(this);
            CheckTitle.append(imageIcon);
            CheckTitle.append(' ');
            CheckTitle.append(this.Title);
            append(dp,CheckTitle);
        case 'CheckResult'
            if(this.status~=ModelAdvisor.CheckStatus.NotRun)

                htmlString=this.ResultInHTML;
                if maObj.ShowActionResultInRpt
                    htmlString=[htmlString,loc_emitActionResult(this)];
                end
                ...
                checkResult.Style=[checkResult.Style,{mlreportgen.dom.OuterMargin('0.15in','0in')}];
                if maObj.IsLibrary&&~maObj.CheckCellArray{this.index}.SupportLibrary&&~modeladvisorprivate('modeladvisorutil2','FeatureControl','ForceRunOnLibrary')
...
function clonedImageIcon=DOM_Get_icon(this)
    icon=ModelAdvisor.CheckStatusUtil.getIcon(this.status,'task');

function varargout=feature(varargin)
    ...
    if isempty(featureName2Desc)
        ...
        featureInfo={...
        'newImpl','use the new LCT''s implementation',featureDefaultValues{1};...
        'supportDynamicArrays','support dynamic arrays specification',featureDefaultValues{2};...
        };
    ...
    elseif strcmpi(featureName,'list')
        ...
        if nargout>0
            ...
            for ii=1:numel(keys)
                ...
                if desktop('-inuse')
                    hyperLink=sprintf(...
                    '(<a href="matlab:legacycode.lct.util.feature(''%s'', %s, true)">%s</a>)',...
    ...
    if nargin>1
        ...
        if nargin>2&&varargin{3}
            fprintf(1,'\n');
            legacycode.lct.util.feature('list');
        end
    end

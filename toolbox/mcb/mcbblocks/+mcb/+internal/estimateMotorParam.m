...
%#codegen
    ...
    if isempty(PhaseDiffValidity)
        PhaseDiffValidity=single(3);
    end

    persistent PhaseDiffValidityLd;
    if isempty(PhaseDiffValidityLd)
        PhaseDiffValidityLd=single(1);
    end

    persistent PhaseDiffValidityLq;
    if isempty(PhaseDiffValidityLq)
        PhaseDiffValidityLq=single(1);
    end
    ...
    persistent PhaseDiffVal;
    if isempty(PhaseDiffVal)
        PhaseDiffVal=single(0);
    end
    ...
    persistent refSignal1;
    if isempty(refSignal1)
        refSignal1=single(0);
    end
    ...
    injFreqMin=single(AlgoParams(2));
    injFreqMax=single(AlgoParams(3));
    ...
    if testNum>0&&testEnable
        ...
        elseif IaIbVdc(3)*inverter_V_max<single(AlgoParams(8))*V_rated
    ...
    case 5
        ...
        delay=single(0.5);
        flag=single(0);
        refSignal=single(AlgoParams(5));
        refSignal1=single(AlgoParams(6));
        testNum=single(2);
    ...
    case 9
        ...
        elseif Sig2>Sig2Mean&&Sig2Prev<Sig2Mean&&count2
            ...
            if(((PhaseDiffVal>single(50*pi/180))||((PhaseDiffVal<single(30*pi/180))&&testNum==3&&PhaseDiffValidityLq)))
                PhaseDiffValidityLq=single(0);
            end
            ...
            if(count1<numInjFreq)
                ...
                estL=estL+tan(phaseDiffRad/numInjFreq)*(Rs+R_board)/(injFreq*2*pi);
    ...
    case 13
        ...
        flag=single(0);
    case 14
        estimatedParameter=bitsll(PhaseDiffValidityLd,1)+PhaseDiffValidityLq;
        estParIdx=single(9);

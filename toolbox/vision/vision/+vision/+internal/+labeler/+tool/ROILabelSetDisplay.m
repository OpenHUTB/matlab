...
classdef ROILabelSetDisplay<vision.internal.labeler.tool.LabelSetDisplay
...
ROIPanelItemVisibility
    ...
    methods(Access=public)
        ...
        function this=ROILabelSetDisplay(hFig,toolName)
            ...
            if~isdeployed()
                ...
                elseif strcmpi(this.ToolName,'lidarLabeler')
                    txt=vision.getMessage('vision:labeler:ROILidarLabelerHelperText');
                else
        ...
        function[textPos,sublabelTextWidth]=subLabelButton(this,textPos,buttonPos,...
            newSublabelIcon)
            ...
            myButtonPos(1)=textPos(1)+sublabelTextWidth-1;
            myButtonPos(2)=this.ButtonOffset;

            this.AddSublabelButton=uicontrol('style','pushbutton','Parent',this.AddLabelPanel,...
            'Units','char',...
            'CData',newSublabelIcon,...
            'ForegroundColor',this.AddButtonTextColor,...
            'Enable','off',...
            'Position',myButtonPos,...
            'TooltipString',vision.getMessage('vision:labeler:AddNewROISublabelToolTip'),...
            'Tag','AddROISublabelButton');

            textPos=[sublabelTextWidth+1,this.ButtonTextOffset,sublabelTextWidth,this.ButtonTextHeight];
            this.AddSublabelText=uicontrol('style','text',...
            'Parent',this.AddLabelPanel,...
            'Units','char',...
            'HorizontalAlignment','left',...
            'Enable','off',...
        ...
        function[myButtonPos,textPos,txt]=attributeButtonPos(this,textPos,buttonPos,sublabelTextWidth)

            myButtonPos=buttonPos;
            myButtonPos(1)=textPos(1)+textPos(3)-1;
            myButtonPos(2)=this.ButtonOffset;

            txt=vision.getMessage('vision:labeler:NewAttributeButtonTitle');
            attrTextWidth=numel(txt)+sum(isspace(txt))+5;
            textPos=[sublabelTextWidth+sublabelTextWidth+1,this.ButtonTextOffset,attrTextWidth,this.ButtonTextHeight];
        end
    end
    ...
    methods
        ...
        function[addLabelPanel,labelSetPanel,buttonPos,helperText]=uicontrolPositions(this)
            ...
            if useAppContainer
                addLabelPanelPixels=hgconvertunits(this.Fig,addLabelPanel,'char','pixels',this.Fig);
            end

            addLabelPanelNormalized=hgconvertunits(this.Fig,addLabelPanel,'char','normalized',this.Fig);
            helperText=[0.05,addLabelPanelNormalized(2)-0.6,0.9,0.5];

...
classdef ShapeLabeler<vision.internal.labeler.tool.ROILabeler
    ...
    methods
            ...
            for i=1:numROIs
                if this.checkROIValidity(this.CurrentROIs{i})
                    if strcmp(this.CurrentROIs{i}.UserData{4},uid)
                        if strcmp(this.CurrentROIs{i}.Type,'images.roi.rectangle')...
                            ||strcmp(this.CurrentROIs{i}.Type,'images.roi.polyline')...
                            ||strcmp(this.CurrentROIs{i}.Type,'images.roi.polygon')...
                            ||strcmp(this.CurrentROIs{i}.Type,'vision.roi.ProjectedCuboid')...
                            ||strcmp(this.CurrentROIs{i}.Type,'images.roi.cuboid')...
            ...
            if nargin==2
                ...
                for i=1:numROIs
                    if~(this.checkROIValidity(this.CurrentROIs{i}))
                        ...
                        elseif strcmp(this.CurrentROIs{i}.Type,'images.roi.rectangle')...
                            ||strcmp(this.CurrentROIs{i}.Type,'images.roi.polyline')...
                            ||strcmp(this.CurrentROIs{i}.Type,'images.roi.polygon')...
                            ||strcmp(this.CurrentROIs{i}.Type,'vision.roi.ProjectedCuboid')...
                            ||strcmp(this.CurrentROIs{i}.Type,'images.roi.cuboid')...
        ...
        function wipeROIs(this)
            ...
            for n=1:numel(this.CurrentROIs)
                if strcmp(this.CurrentROIs{n}.Type,'images.roi.rectangle')...
                    ||strcmp(this.CurrentROIs{n}.Type,'images.roi.polyline')...
                    ||strcmp(this.CurrentROIs{n}.Type,'images.roi.polygon')...
                    ||strcmp(this.CurrentROIs{n}.Type,'vision.roi.ProjectedCuboid')...
                    ||strcmp(this.CurrentROIs{n}.Type,'images.roi.cuboid')...
        ...
        function rois=getSelectedROIsForMove(this)
            rois=getSelectedROIsForCopy(this);
        end
        ...
        function moveSelectedROI(this,roiInfo,keyPressed)
            ...
            elseif isa(roiInfo,'vision.roi.ProjectedCuboid')
        ...
        function setROIColorByGroup(this,val,roiLabelDefinitionStruct)
            ...
            COLOR_COUNT=length(this.ROI_INSTANCE_COLOR);

            colors=this.ROI_INSTANCE_COLOR;



            labelColorLookup=containers.Map;
            for labelIdx=1:numel(roiLabelDefinitionStruct)
                labelColorLookup(roiLabelDefinitionStruct(labelIdx).Name)=...
                roiLabelDefinitionStruct(labelIdx).Color;
            end

            roiCount=0;
            for roiIdx=1:numel(this.CurrentROIs)

                [labelName,~,~,~]=getLabelDefInfo(this,this.CurrentROIs{roiIdx});

                if strcmp(val,'By Label')
                    this.CurrentROIs{roiIdx}.Color=labelColorLookup(labelName);
                else
                    labelColor=colors(mod(roiCount,COLOR_COUNT)+1,:);
                    this.CurrentROIs{roiIdx}.Color=labelColor;
                end
        ...
        function color=getNextInstanceColor(this)
            ...
            color=colors(mod(numROIs+1,COLOR_COUNT)+1,:);

        end


        function color=getInstanceColorByIdx(this,idx)

            COLOR_COUNT=length(this.ROI_INSTANCE_COLOR);
            colors=this.ROI_INSTANCE_COLOR;

            color=colors(mod(idx,COLOR_COUNT)+1,:);
        ...
        function TF=isParentROISelected(~,selectedROIData,sublabelItemData)
            TF=strcmpi(selectedROIData.Tag,sublabelItemData.LabelName)&&...
            isempty(selectedROIData.UserData{2});
        end


        function TF=isSelfOrSiblingROISelected(~,selectedROIData,sublabelItemData)
            TF=strcmpi(selectedROIData.UserData{2},sublabelItemData.LabelName);
        end


        function N=getNumSelectedROIs(this)
            N=0;
            for i=1:numel(this.CurrentROIs)
                N=N+double(this.CurrentROIs{i}.Selected);
            end
        end
        ...
        function[labelName,sublabelName,selfUID,parentUID]=getLabelDefInfoFromUID(this,uid)

            for i=1:numel(this.CurrentROIs)
                if strcmp(this.CurrentROIs{i}.UserData{4},uid)

                    data=this.getCopiedData(this.CurrentROIs{i});
                    ud=data.UserData;
                    parentName=ud{2};
        ...
        function[numROIselected,singleSelectedROIInfo]=selectedROIInstanceInfo(this)
            ...
            for i=1:numel(this.CurrentROIs)
                if this.CurrentROIs{i}.Selected
                    ...
                    numROIselected=numROIselected+1;
                    singleSelectedROIInfo.SelfUID=selfUID;
                    singleSelectedROIInfo.ParentUID=parentUID;
                    singleSelectedROIInfo.LabelName=labelName;
                    singleSelectedROIInfo.SublabelName=sublabelName;
                    if(numROIselected>1)
                        numROIselected=inf;
                        singleSelectedROIInfo.SelfUID='';
                        singleSelectedROIInfo.ParentUID='';
        ...
        function[roiName,parentName,parentUID]=getSelectedItemDefROIInstanceInfo(this)
            ...
            [labelName,labelUID]=getLastSelectedLabelInstanceInfo(this);
            if isempty(parentName)
                ...
                parentUID=labelUID;
                if~strcmp(parentName,labelName)

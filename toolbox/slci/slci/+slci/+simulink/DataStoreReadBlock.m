...
classdef DataStoreReadBlock<slci.simulink.Block
    ...
    methods
        ...
        function obj=DataStoreReadBlock(aBlk,aModel)
    ...
    methods(Static=true)
        ...
        function resolvedType=getResolvedType(typeName,dsmHandle)
            if slci.internal.isSimulinkBuildInType(typeName)
                resolvedType=typeName;
            else

                if~isequal(dsmHandle,-1)
                    try
                        resolvedType=slResolve(typeName,dsmHandle);
                        return;
                    catch
                    end
                end

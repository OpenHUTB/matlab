...
classdef StateflowLoopInitConstraint<slci.compatibility.Constraint


    methods

        function out=getDescription(aObj)%#ok
            out=['A supported stateflow loop entry transition'...
            ,'must define only one action that initializes the '...
            ,'scalar induction variable with a literal value.'];

        end


        function obj=StateflowLoopInitConstraint
            obj.setEnum('StateflowLoopInit');
        ...
        function out=check(aObj)
            ...
            isSupported=aObj.getOwner().isSupportedLoopInit();
            if~isSupported

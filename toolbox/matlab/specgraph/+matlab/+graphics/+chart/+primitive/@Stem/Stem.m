...
ZDataCache
    ...
    methods
        function hObj=Stem(varargin)
            ...
            set(hObj.MarkerHandle,'Description_I','Stem MarkerHandle');

            set(hObj.MarkerHandle,'Internal',true);

            hObj.Edge_I=matlab.graphics.primitive.world.LineStrip;

            set(hObj.Edge,'Description_I','Stem Edge');
    ...
    methods(Access='private',Hidden=true)
        function doSetup(hObj)
            ...
            addDependencyConsumed(hObj,{'baseline','dataspace','colororder_linestyleorder'});



            hObj.linkDataPropertyToChannel('XData','X','Y');
            hObj.linkDataPropertyToChannel('YData','Y');
            hObj.linkDataPropertyToChannel('ZData','Z');
        end
    end
    methods(Access='public',Hidden=true)
    ...
    methods(Access='protected',Hidden=true)
        function doBrushing(hObj)
            ...
            if~isempty(hObj.BrushHandles)
                ...
                hObj.BrushHandles=matlab.graphics.chart.primitive.stem.StemBrushing;
    ...
    methods(Access='protected',Hidden=true)
        function varargout=getPropertyGroups(hObj)
            ...
            'MarkerFaceColor','BaseValue'},dnames]);
        end
    end
    methods(Access='protected',Hidden=true)

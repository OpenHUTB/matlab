function[summaryLine,xyzLabelCode]=generateXYLabelAndComments(chartName,varNames,zVar)
...
function xyLabelCode=getXYLabelCode(firstVar,secondVar,zVar)
    ...
    if~isempty(secondVar)
        commentCode=['% ',getString(message("MATLAB:graphics:visualizedatatask:CommentForAnnotations",'xlabel, ylabel, title','legend'))];
    ...
    elseif~isempty(firstVar)&&isempty(zVar)
        commentCode=['% ',getString(message("MATLAB:graphics:visualizedatatask:CommentForAnnotations",'ylabel, title','legend'))];
    ...
    if~isempty(zVar)
        ...
        if isempty(commentCode)
            commentCode=['% ',getString(message("MATLAB:graphics:visualizedatatask:CommentForAnnotations",'zlabel, title','legend'))];
        else

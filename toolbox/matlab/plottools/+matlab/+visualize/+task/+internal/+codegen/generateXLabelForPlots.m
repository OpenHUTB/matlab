function generatedCode=generateXLabelForPlots(chartName,outputs,channels,parameters,doGenerateLabels)
    ...
    for i=1:numel(channels)
        ...
        elseif~strcmpi(dataMapped,'select variable')&&~strcmpi(dataMapped,'default value')
            if channel.IsRequired&&strcmpi(chartName,'histogram')
    ...
    if strcmpi(chartName,'histogram')
        labelCode=[labelCode,newline,'legend'];
    end
    ...
    if~isempty(parameterCode)
        vizCode=[vizCode,parameterCode,displayStringCode];
    else
        vizCode=[vizCode,displayStringCode];
    end

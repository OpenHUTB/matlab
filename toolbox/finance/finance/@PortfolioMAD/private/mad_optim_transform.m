function[AI,bI,AE,bE,lB,uB,f0,f,x0]=mad_optim_transform(obj)
    ...
    if sum(~isfinite(bI)&(bI~=Inf))>0
        error(message('finance:PortfolioMAD:mad_optim_transform:InvalidUpperBoundConstraint'));
    end
    ...
    if isempty(obj.sampleAssetMean)
        error(message('finance:PortfolioMAD:mad_optim_transform:MissingScenarios'));
    end
    ...
    if obj.checkBounds||any(~isfinite(lB))
        [glb,isbounded]=mad_estimate_lower_bound(obj.NumAssets,[AI;AE;-AE],[bI;bE;-bE],...
        lB,[],obj.solverOptionsLP,obj.usePresolver);
        lB(1:n)=glb;
    ...
    if isempty(isbounded)
        if~isempty(obj.Turnover)||~isempty(obj.BuyTurnover)||~isempty(obj.SellTurnover)
            error(message('finance:PortfolioMAD:mad_optim_transform:EmptySetWithTurnover'));
        else
            error(message('finance:PortfolioMAD:mad_optim_transform:EmptySet'));
        end
    end

    if~isbounded
        error(message('finance:PortfolioMAD:mad_optim_transform:UnboundedFromBelow'));
    end

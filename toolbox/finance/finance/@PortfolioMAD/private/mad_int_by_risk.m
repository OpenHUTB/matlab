function[pwgt,status]=mad_int_by_risk(obj,ProbStruct,risk)
    ...
    nonLinObj=@(x)mad_return_as_objective(x,ProbStruct.RetnLin,...
    solver.ObjectiveScalingFactor);
    nonLinConFcn=@(x)mad_function_as_constraint(x,dY,risk,...
    ...
    feasNLCons=@(x)mad_feasibilityRiskConstraint(x,dY,risk,...
    solver.NonlinearScalingFactor);
    NLPFeasProb=@(intVar,x0)fmincon(@mad_feasibilityObjective,x0,...

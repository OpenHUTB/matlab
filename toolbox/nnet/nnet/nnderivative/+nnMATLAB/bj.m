function[JE,JJ,Perfs,PerfN]=bj(net,X,Xi,Pc,Pd,Ai,T,EW,masks,Q,TS,hints)
    ...
    NoTS=hints.numOutputElements*TS;
    for i=1:net.numLayers
        if net.biasConnect(i)
            dB{i}=zeros(net.layers{i}.size,Q,NoTS);
        end
        for j=1:net.numInputs
            if net.inputConnect(i,j)
                wsize=net.inputWeights{i,j}.size;
                dIW{i,j}=zeros(wsize(1),wsize(2),Q,NoTS);
            end
        end
        for j=1:net.numLayers
            if net.layerConnect(i,j)
                wsize=net.layerWeights{i,j}.size;
                dLW{i,j}=zeros(wsize(1),wsize(2),Q,NoTS);
            end
        end
    end
    ...
    for i=1:net.numLayers
        dA(i,:)={zeros(net.layers{i}.size,Q,NoTS)};
    end
    ...
    for ts=TS:-1:1
        for i=fliplr(hints.layerOrder)
            ...
            elseif hints.tfTansig(i)
                ai=Ac{i,a_ts};
                dn=bsxfun(@times,dA{i,ts},(1-(ai.*ai)));
            else
            ...
            for j=net.numLayers:-1:1
                if net.layerConnect(i,j)
                    ...
                    if hints.lwInclude(i,j)
                        if hints.lwDotprod(i,j)
                            Rd=size(net.LW{i,j},2);
                            dLW{i,j}=dLW{i,j}+bsxfun(@times,reshape(dz,net.layers{i}.size,1,Q,NoTS),reshape(ad,1,Rd,Q));
                        else
                    ...
                    if hints.lwDotprod(i,j)
                        Rd=size(net.LW{i,j},2);
                        dad=reshape(net.LW{i,j}'*reshape(dz,net.layers{i}.size,Q*NoTS),Rd,Q,NoTS);
                    else
            ...
            for j=net.numInputs:-1:1
                if net.inputConnect(i,j)
                    ...
                    if hints.iwInclude(i,j)
                        ...
                        if hints.iwDotprod(i,j)
                            Rd=size(net.IW{i,j},2);
                            dIW{i,j}=dIW{i,j}+bsxfun(@times,reshape(dz,net.layers{i}.size,1,Q,NoTS),reshape(pd,1,Rd,Q));
                        else

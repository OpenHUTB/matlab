...
classdef replay_block<wt.internal.uhd.mcos.block
...
repeatedTransmissionChannels
    ...
    methods
        function obj=replay_block(radio,blockName,varargin)
            obj=obj@wt.internal.uhd.mcos.block(blockName,varargin);
            obj.ctrl=uhd.internal.replay;
            makeBlock(obj,radio);
            obj.word_size=obj.ctrl.getWordSize();
            obj.mem_size=obj.ctrl.getMemSize();
        end
    end
    ...
    methods
        ...
        function flushReplayMemory(obj,channel)
            fullness=obj.getRecordFullness(channel);
            retriesLeft=500;


            while(fullness)&&retriesLeft
                obj.ctrl.recordRestart(channel);
                pause(0.01);
                fullness=obj.getRecordFullness(channel);
        ...
        function timed_replay_start(obj,~,mode,len,channelList,time)




            obj.ctrl.setTimeSpec(time);
            for n=1:length(channelList)
                obj.issueStreamCommand(mode,len,channelList(n),time);
            end
            pause(time);
        end
        ...
        function configPlay(obj,offset,size,port)
            obj.ctrl.configPlay(offset,size,port);
        end
        ...
        function record(obj,offset,size,port)
            obj.ctrl.record(offset,size,port);
        end
        ...
        function val=getRecordFullness(obj,varargin)
            ...
            val=obj.ctrl.getRecordFullness(port);
        end

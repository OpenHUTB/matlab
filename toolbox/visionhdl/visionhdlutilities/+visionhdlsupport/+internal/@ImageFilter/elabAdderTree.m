function treeAddOutSig=elabAdderTree(this,hN,...
    addInSym,addInASym,sigName)%#ok<INUSL>
    ...
    while(numel(treeSignalIn)>1)
        ...
        for ii=1:floor(numel(treeSignalIn)/2)
            ...
            if(stageNum~=1)||...
                ...
                pirelab.getAddComp(hN,[in1RegOut,in2RegOut],addOutSig);
                treeSignalOut(ii)=addOutSig;
            elseif(in1_op=='+'&&in2_op=='-')


                pirelab.getSubComp(hN,[in1RegOut,in2RegOut],addOutSig);
                treeSignalOut(ii)=addOutSig;
...
...
...
...
...
        ...
        if mod(numel(treeSignalIn),2)
            ...
            treeSignalOut(end+1)=in1RegOut;%#ok<AGROW>
        end
    ...
    pirelab.getUnitDelayComp(hN,treeSignalOut,treeAddOutSig,[sigName,'_final']);

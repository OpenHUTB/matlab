function topNet=elaborateMultiPixelEdgeDetector(this,topNet,blockInfo,insignals,outsignals)
    ...
    case 'Sobel'
        fracLen=3;
        wordLen=3;
        blockInfo.pixelInVecDT=pirelab.createPirArrayType(dataType.BaseType,[3,dataType.Dimensions]);
    ...
    otherwise
        fracLen=1;
        wordLen=1;
        blockInfo.pixelInVecDT=pirelab.createPirArrayType(dataType.BaseType,[2,dataType.Dimensions]);
    end


    blockInfo.gradType=topNet.getType('FixedPoint',...
    'Signed',true,...
    'WordLength',dataType.BaseType.WordLength+wordLen,...
    'FractionLength',dataType.BaseType.FractionLength-fracLen);


    if blockInfo.GradientComponentOutputPorts
        if strcmp(blockInfo.GradientDataType,'Same as first input')
            blockInfo.gradType=dataType.BaseType;
    ...
    gradMultiPixelType=pirelab.createPirArrayType(blockInfo.gradType,[1,dataType.Dimensions]);
    blockInfo.gradMultiPixelType=gradMultiPixelType;
    g1Out=topNet.addSignal(gradMultiPixelType,'gradcomp1');
    g2Out=topNet.addSignal(gradMultiPixelType,'gradcomp2');
    switch blockInfo.Method
    case 'Sobel'


        sobelcoreNet=this.elabMultiPixelSobelCore(topNet,blockInfo,inRate);
        ...
        coreDelayBal=5;
    case 'Prewitt'
        prewittcoreNet=this.elabMultiPixelPrewittCore(topNet,blockInfo,inRate);
        ...
        coreDelayBal=9;
    case 'Roberts'
        robertscoreNet=this.elabMultiPixelRobertsCore(topNet,blockInfo,inRate);
    ...
    if blockInfo.BinaryImageOutputPort
        ...
        g1split=g1Out.split.PirOutputSignals;
        g2split=g2Out.split.PirOutputSignals;

        for ii=1:1:blockInfo.NumberOfPixels
            BImageOut(ii)=topNet.addSignal(ctrlType,'edge');
            pirelab.instantiateNetwork(topNet,bimageNet,[g1split(ii),g2split(ii),threshsq],...
            BImageOut(ii),'BinaryImageNet_inst');
            binaryDelayBal=6;
        end
    ...
    if blockInfo.GradientComponentOutputPorts
        g1OutDelay=topNet.addSignal(blockInfo.gradMultiPixelType,'g1OutDelay');
        g2OutDelay=topNet.addSignal(blockInfo.gradMultiPixelType,'g2OutDelay');
        if binaryDelayBal>0
        ...
        zeroconst=topNet.addSignal(blockInfo.gradMultiPixelType,'const_zero');
        pirelab.getConstComp(topNet,zeroconst,0);

        switchout1=topNet.addSignal(blockInfo.gradMultiPixelType,'g1OutNext');
        pirelab.getSwitchComp(topNet,[zeroconst,g1OutDelay],switchout1,vlODelay);
        pirelab.getUnitDelayComp(topNet,switchout1,Grad1);

        switchout2=topNet.addSignal(blockInfo.gradMultiPixelType,'g2OutNext');
    ...
    if blockInfo.BinaryImageOutputPort
        ...
        for ii=1:1:blockInfo.NumberOfPixels
            edgeNext(ii)=topNet.addSignal(ctrlType,'edgeNext');
            edgeDelay(ii)=topNet.addSignal(ctrlType,'edgeDelay');
            pirelab.getLogicComp(topNet,[vlODelay,BImageOut(ii)],edgeNext(ii),'and');
            pirelab.getUnitDelayComp(topNet,edgeNext(ii),edgeDelay(ii));
        end
        pirelab.getMuxComp(topNet,edgeDelay(:),Edge);

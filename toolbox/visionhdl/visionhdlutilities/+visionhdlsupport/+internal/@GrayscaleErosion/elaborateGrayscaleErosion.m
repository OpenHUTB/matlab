function elaborateGrayscaleErosion(this,topNet,blockInfo,inSignals,outSignals)
    ...
    if strcmpi(blockInfo.Algorithm,'fullTreeErosion')
    ...
    elseif strcmpi(blockInfo.Algorithm,'decompositionErosion')
        ...
        lineBufferBlockInfo.BiasUp=true;
        lineBufferBlockInfo.MaxLineSize=blockInfo.LineBufferSize;
        lineBufferBlockInfo.Algorithm='vanHerkErosion';
        lbufVType=pirelab.getPirVectorType(inType,blockInfo.kHeight);
        sigInfo.lbufVType=lbufVType;
        dataVectorOut=topNet.addSignal2('Type',inType,'Name','dataVectorOut');
        lineBufNet=this.addLineBuffer(topNet,lineBufferBlockInfo,dataRate,inType);
        lineBufferOut=[dataVectorOut,hStartOutLB,hEndOutLB,vStartOutLB,vEndOutLB,validOutLB,processDataOut];
    else
        lineBufferBlockInfo.KernelHeight=blockInfo.kHeight;
        ...
        lineBufferBlockInfo.BiasUp=true;
        lineBufferBlockInfo.MaxLineSize=blockInfo.LineBufferSize;
        dataVectorOut=topNet.addSignal2('Type',inType,'Name','dataOut');
        lineBufNet=this.addLineBuffer(topNet,lineBufferBlockInfo,dataRate,inType);
        lineBufferOut=[dataVectorOut,hStartOutLB,hEndOutLB,vStartOutLB,vEndOutLB,validOutLB,processDataOut];
    end
    ...
    if strcmpi(blockInfo.Algorithm,'fullTreeErosion')
        coreNet=this.elaboratefullTreeErosion(topNet,blockInfo,sigInfo,dataRate);
        coreIn=[dataVectorOut,hStartOutLB,hEndOutLB,vStartOutLB,vEndOutLB,validOutLB,processDataOut];
    elseif strcmpi(blockInfo.Algorithm,'decompositionErosion')
        coreNet=this.elaborateDecompositionErosion(topNet,blockInfo,sigInfo,dataRate);
        coreIn=[dataVectorOut,hStartOutLB,hEndOutLB,vStartOutLB,vEndOutLB,validOutLB,processDataOut];
    else
        coreNet=this.elaborateVanHerkErosion(topNet,blockInfo,sigInfo,dataRate);
        coreIn=[dataVectorOut,hStartOutLB,hEndOutLB,vStartOutLB,vEndOutLB,validOutLB,processDataOut];
    ...
    core=pirelab.instantiateNetwork(topNet,coreNet,coreIn,coreOut,'erosionCore');
    core.addComment('erosionCore');

function cNet=elabErosionCore(~,topNet,blockInfo,inRate)
    ...
    cNet.addComment('Final local minima in binary image');
    ...
    for i=1:kW
        ...
        if i==1
            comp.addComment('Finding local minima');
        end
    ...
    finaland=cNet.addSignal(boolType,'finaand');
    pirelab.getLogicComp(cNet,and_col,finaland,'and');





    linebufferDelay=floor(kW/2);
    ...
    if strcmpi(blockInfo.PaddingMethod,'None')
        ...
        pirelab.getSwitchComp(cNet,[finaland,falseout],validpixel,pixelSelValidOut,'','==',1);

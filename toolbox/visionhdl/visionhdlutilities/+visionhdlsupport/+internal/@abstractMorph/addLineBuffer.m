function lNet=addLineBuffer(~,topNet,blockInfo,inRate)
    ...
    if isfield(dI,'Dimensions')
        lbufVType=pirelab.createPirArrayType(boolType,[blockInfo.KernelHeight,topNet.PirInputSignals(1).Type.Dimensions]);
    else
        lbufVType=pirelab.getPirVectorType(boolType,blockInfo.KernelHeight);
    end
    ...
    'InportTypes',[topNet.PirInputSignals(1).Type,boolType,boolType,boolType,boolType,boolType],...

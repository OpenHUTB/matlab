function elaborateDilation(this,topNet,blockInfo,inSig,outSig)
    ...
    lbufInfo.NumberOfPixels=blockInfo.NumberOfPixels;
    ...
    if inputDim==1
        cNet=this.elabDilationCore(topNet,blockInfo,inRate);
        cNet.addComment('Find local maxima in binary image');
    else
        cNet=this.elabMultiPixelDilationCore(topNet,blockInfo,inRate);
        cNet.addComment('Find local maxima in binary image');
    end

classdef(StrictDefaults)Dilation<visionhdl.internal.abstractLineMemoryKernel
...
%#ok<*EMCLS>
    ...
    methods(Static,Access=protected)
        function header=getHeaderImpl
            ...
            header=matlab.system.display.Header('visionhdl.Dilation',...
    ...
    methods
        function obj=Dilation(varargin)
        ...
        function set.Neighborhood(obj,val)
            ...
            validateattributes(val,{'logical','double','single'},{'2d','binary'},'Dilation','Neighborhood');
            [height,width]=size(val);
            validateattributes(height,{'numeric'},{'scalar','<=',32,'>',0},'Dilation','first dimension of Neighborhood');
            validateattributes(width,{'numeric'},{'scalar','<=',32},'Dilation','second dimension of Neighborhood');
        ...
        function set.LineBufferSize(obj,val)
            validateattributes(val,{'numeric'},{'integer','scalar','>',0},'Dilation','MaxLineSize');
    ...
    methods(Access=protected)
        ...
        function validateInputsImpl(~,pixelIn,ctrlIn)
            ...
            if isempty(coder.target)||~eml_ambiguous_types
                ...
                validateattributes(pixelIn,{'double','single','logical'},{'binary'},'Dilation','pixel input');
        ...
        function setupImpl(obj,pixelIn,ctrlIn)
            ...
            if obj.NumberOfPixels==1
                obj.filterHandle=@ComputeDilation;
            else
                obj.filterHandle=@ComputeDilationMultiPixel;
            end
            ...
            obj.KernelMemoryPaddingValue=0;
            ...
            obj.linebufferDelay=floor((obj.kWidth-1)/2);
            ...
            if obj.NumberOfPixels>1
                ...
                if obj.kWidth==2||obj.kHeight==2||obj.kHeight==1||obj.kWidth==1
        ...
        function mval=ComputeDilation(obj,dataCol,~)
            ...
            if obj.PaddingNoneValid
                ...
                anddata=false(obj.kHeight,obj.kWidth);
                firstor=false(1,obj.kWidth);
                finalor=false;
                for i=1:obj.kWidth
                    for j=1:obj.kHeight
                        anddata(j,i)=and(padNoneKernel(j,i),obj.Neighborhood(j,i));
                        firstor(1,i)=or(firstor(1,i),anddata(j,i));
                    end
                    finalor=or(finalor,firstor(1,i));
                end
                ...
                for i=1:obj.kWidth
                    for j=1:obj.kHeight
                        ...
                        anddata(j,i)=and(obj.dataColReg(j,i),obj.Neighborhood(j,i));
                        firstor(1,i)=or(firstor(1,i),anddata(j,i));
                    end
                    finalor=or(finalor,firstor(1,i));
                end
        ...
        function mval=ComputeDilationMultiPixel(obj,dataVector,ctrlIn)

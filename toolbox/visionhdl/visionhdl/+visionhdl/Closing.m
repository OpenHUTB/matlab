classdef(StrictDefaults)Closing<visionhdl.internal.abstractLineMemoryKernel
...
%#ok<*EMCLS>
    ...
    methods(Static,Access=protected)
        function header=getHeaderImpl
            ...
            header=matlab.system.display.Header('visionhdl.Closing',...
    ...
    methods
        ...
        function set.Neighborhood(obj,val)
            ...
            validateattributes(val,{'logical','double','single'},{'2d','binary'},'Closing','Neighborhood');
            [height,width]=size(val);
            validateattributes(height,{'numeric'},{'scalar','<=',32,'>',0},'Closing','first dimension of Neighborhood');
            validateattributes(width,{'numeric'},{'scalar','<=',32},'Closing','second dimension of Neighborhood');
        ...
        function set.LineBufferSize(obj,val)
            validateattributes(val,{'numeric'},{'integer','scalar','>',0},'Closing','MaxLineSize');
    ...
    methods(Access=protected)
        function validateInputsImpl(~,pixelIn,ctrlIn)
            ...
            if isempty(coder.target)||~eml_ambiguous_types
                ...
                validateattributes(pixelIn,{'double','single','logical'},{'binary'},'Closing','pixel input');

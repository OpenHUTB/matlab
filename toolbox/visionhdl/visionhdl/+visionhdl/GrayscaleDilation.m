classdef(StrictDefaults)GrayscaleDilation<visionhdl.internal.abstractLineMemoryKernel
...
%#ok<*EMCLS>
    ...
    methods(Static,Access=protected)
        function header=getHeaderImpl
            ...
            header=matlab.system.display.Header('visionhdl.GrayscaleDilation',...
            'ShowSourceLink',false,...
            'Title','Grayscale Dilation');
        end

    end

    methods
        function obj=GrayscaleDilation(varargin)
    ...
    methods(Access=protected)
        ...
        function s=saveObjectImpl(obj)
            ...
            if obj.isLocked
                ...
                s.hVanHerkDilation=obj.hVanHerkDilation;
    ...
    methods(Access=private)
        ...
        function vanHerkDilation(obj,pixelIn,controlIn)
            ...
            if processData||processOut
                obj.vanHerkOutPipe(1)=max(obj.forwardStream(4),obj.backwardStream(1));
            else
            ...
            if validOutBuffer
                obj.vanHerkValidREG(:)=obj.vanHerkOutPipe(1);
            else
        ...
        function[pixelOut]=vanHerkForwardMax(obj,pixelIn,~,processDataIn)
            ...
            if processDataIn
                ...
                if moduloReached
                    ...
                    obj.vanHerkForwardCurrentMaximum(:)=max(pixelIn,obj.vanHerkForwardCurrentMaximum);
                end
            ...
            if processDataIn&&obj.vanHerkForwardBufferComplete
                obj.vanHerkOutREG(:)=obj.vanHerkForwardBuffer(obj.vanHerkForwardFIFOReadCounter+1);
                if obj.vanHerkForwardFIFOReadCounter<length(obj.Neighborhood)
        ...
        function[pixelOut]=vanHerkBackwardMax(obj,pixelIn,~,processDataIn,forwardBufferFlag)
            ...
            elseif obj.vanHerkLineEnd
                ...
                obj.vanHerkBackwardCurrentMaximum(:)=max(obj.vanHerkBackwardCurrentMaximum,obj.vanHerkMirrorOutput);
            end
        ...
        function decompositionDilation(obj,pixelIn,controlIn)
            [pixelVanHerkOut,controlVanHerkOut]=step(obj.hVanHerkDilation,pixelIn,controlIn);
            ...
            obj.decompositionDataReg(1)=max(dataCol(:));
        ...
        function fullTreeDilation(obj,pixelIn,controlIn)
            ...
            colMax=max(currentPOCI(:,:));
            obj.vanHerkOutREG(1)=max(colMax);
        ...
        function setupFloat(obj,type,NLength,NHeight)
            ...
            if obj.kHeight>1&&any(obj.Neighborhood(:)==false)
                obj.vanHerkControlBuffer=false(6,1+ceil(log2(NHeight))+ceil(log2(NLength)));
            ...
            elseif obj.kHeight>1&&all(obj.Neighborhood(:)==true)
                ...
                obj.vanHerkOutputData=cast(zeros(1,8),type);
                obj.hVanHerkDilation=visionhdl.GrayscaleDilation(ones(1,NLength));
                pipeDelay=ceil(log2(obj.kHeight));

                if mod(obj.kWidth,2)==0
                    obj.decompositionDataReg=cast(zeros(1,pipeDelay+4),type);
                    obj.decompositionControlReg=cast(zeros(6,pipeDelay+4),type);
                else
                ...
                obj.vanHerkOutputControl=false(6,7);
                obj.dataColReg=cast(zeros(1,3),type);
                obj.vanHerkOutputData=cast(zeros(1,14),type);
            ...
            if(obj.kHeight>1&&any(obj.Neighborhood(:)==false))
                obj.vanHerkOutREG=cast(zeros(1,2+ceil(log2(NHeight))+ceil(log2(NLength))),type);
            elseif(obj.kHeight>1&&all(obj.Neighborhood(:)==true))
                ...
                obj.vanHerkOutPipe=cast(0,type);
            end
        ...
        function setupFixed(obj,WL,FL,NLength,NHeight)
            ...
            if obj.kHeight>1&&any(obj.Neighborhood(:)==false)||((obj.kWidth<8)&&(obj.kWidth>1))


                obj.vanHerkControlBuffer=false(6,1+ceil(log2(NHeight))+ceil(log2(NLength)));
                obj.dataColReg=(fi((zeros(NHeight,NLength)),0,WL,FL,'RoundingMethod','Nearest','OverflowAction','Saturate','ProductMode','FullPrecision'));
            elseif obj.kHeight>1&&obj.kWidth==1
                obj.vanHerkControlBuffer=false(6,ceil(NLength/2)+ceil(log2(NHeight))+ceil(log2(NLength)));
                obj.dataColReg=(fi((zeros(NHeight,NLength)),0,WL,FL,'RoundingMethod','Nearest','OverflowAction','Saturate','ProductMode','FullPrecision'));
            ...
            elseif obj.kHeight>1&&all(obj.Neighborhood(:)==true)
                ...
                obj.vanHerkOutputData=(fi((zeros(1,8)),0,WL,FL,'RoundingMethod','Nearest','OverflowAction','Saturate','ProductMode','FullPrecision'));
                obj.hVanHerkDilation=visionhdl.GrayscaleDilation(ones(1,NLength));
                pipeDelay=ceil(log2(obj.kHeight));

                if mod(obj.kWidth,2)==0
                    obj.decompositionDataReg=(fi(zeros(1,pipeDelay+4),0,WL,FL,'RoundingMethod','Nearest','OverflowAction','Saturate','ProductMode','FullPrecision'));
                    obj.decompositionControlReg=(fi(zeros(6,pipeDelay+4),0,WL,FL,'RoundingMethod','Nearest','OverflowAction','Saturate','ProductMode','FullPrecision'));
                else
                ...
                obj.vanHerkOutputControl=false(6,7);
                obj.dataColReg=(fi((zeros(1,3)),0,WL,FL,'RoundingMethod','Nearest','OverflowAction','Saturate','ProductMode','FullPrecision'));
                obj.vanHerkOutputData=(fi((zeros(1,14)),0,WL,FL,'RoundingMethod','Nearest','OverflowAction','Saturate','ProductMode','FullPrecision'));
            ...
            obj.vanHerkOutputMirrorPingCounter=uint32(0);
            obj.vanHerkOutputMirrorPongCounter=uint32(0);
            if(obj.kHeight>1&&any(obj.Neighborhood(:)==false))&&obj.kWidth>1
                obj.vanHerkOutREG=(fi(zeros(1,ceil(log2(NHeight))+2+ceil(log2(NLength))),0,WL,FL,'RoundingMethod','Nearest','OverflowAction','Saturate','ProductMode','FullPrecision'));
            elseif(obj.kHeight>1&&all(obj.Neighborhood(:)==true))&&obj.kWidth>1
            ...
            elseif(obj.kHeight>1)&&obj.kWidth==1
                ...
                obj.vanHerkOutPipe=(fi(zeros(1,3),0,WL,FL,'RoundingMethod','Nearest','OverflowAction','Saturate','ProductMode','FullPrecision'));
        ...
        function setupInteger(obj,WL,NLength,NHeight)
            ...
            elseif obj.kHeight>1&&all(obj.Neighborhood(:)==true)
                ...
                obj.vanHerkOutputData=storedInteger(fi((zeros(1,8)),0,WL,0,'RoundingMethod','Nearest','OverflowAction','Saturate','ProductMode','FullPrecision'));
                obj.hVanHerkDilation=visionhdl.GrayscaleDilation(ones(1,NLength));
                pipeDelay=ceil(log2(obj.kHeight));

                if mod(obj.kWidth,2)==0
                    obj.decompositionDataReg=storedInteger(fi(zeros(1,pipeDelay+4),0,WL,0,'RoundingMethod','Nearest','OverflowAction','Saturate','ProductMode','FullPrecision'));
                    obj.decompositionControlReg=storedInteger(fi(zeros(6,pipeDelay+4),0,WL,0,'RoundingMethod','Nearest','OverflowAction','Saturate','ProductMode','FullPrecision'));
                else
                ...
                obj.vanHerkOutputControl=false(6,7);
                obj.dataColReg=storedInteger(fi((zeros(1,3)),0,WL,0,'RoundingMethod','Nearest','OverflowAction','Saturate','ProductMode','FullPrecision'));
                obj.vanHerkOutputData=storedInteger(fi((zeros(1,14)),0,WL,0,'RoundingMethod','Nearest','OverflowAction','Saturate','ProductMode','FullPrecision'));
            ...
            obj.vanHerkOutputMirrorPongCounter=uint32(0);
            if(obj.kHeight>1&&any(obj.Neighborhood(:)==false))&&obj.kWidth>1
                obj.vanHerkOutREG=storedInteger(fi(zeros(1,ceil(log2(NHeight))+2+ceil(log2(NLength))),0,WL,0,'RoundingMethod','Nearest','OverflowAction','Saturate','ProductMode','FullPrecision'));
            elseif(obj.kHeight>1&&all(obj.Neighborhood(:)==true))&&obj.kWidth>1
            ...
            elseif(obj.kHeight>1)&&obj.kWidth==1
                ...
                obj.vanHerkOutPipe=storedInteger(fi(zeros(1,3),0,WL,0,'RoundingMethod','Nearest','OverflowAction','Saturate','ProductMode','FullPrecision'));

...
function simInEvaluateAndSimulateV2(simInStructCellArray,simWatchersCellArray,useParallel,resultSetId)
    ...
    [runCfgArray,simInStructCellArray,simWatchersCellArray]=stm.internal.SimulationInput.constructRunCfgArrayV2(simInStructCellArray,simWatchersCellArray,useParallel);
    ...
    stm.internal.SimulationInput.simulateAndEvaluateV2(runCfgArray,simInStructCellArray,simWatchersCellArray,useParallel,resultSetId);
...
function out=getModelHarnessName(simInStructCellArray)
    ind=strfind(simInStructCellArray.HarnessName,'%%%');
    if~isempty(ind)&&~isequal(ind,1)
        harnessName=simInStructCellArray.HarnessName(1:ind(1)-1);
    else
        harnessName='';
    end
    out=strcat(simInStructCellArray.Model,harnessName);
end

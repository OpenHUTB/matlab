function[table,rowNumber]=getHTMLGlobvalVariableStructTable(obj,members,lvl,id,rowNumber,hasMdlRefVars,colWidthsInPercent,colAlignment)
    ...
    for i=1:length(members)
        ...
        col3={[int2str(members(i).UseCount),'<span style="font-family:Courier">&#160;</span>']};
        memberUseCountInFcn=0;
        if~isempty(members(i).UseInFunctions)
            memberUseCountInFcn=max([members(i).UseInFunctions.UseCount]);
        end
        col4={[int2str(memberUseCountInFcn),'<span style="font-family:Courier">&#160;</span>']};
        if mod(rowNumber,2)
        ...
        if isempty(members(i).Members)
            ...
            [struct_table,rowNumber]=obj.getHTMLGlobvalVariableStructTable(members(i).Members,lvl,id,rowNumber,hasMdlRefVars,colWidthsInPercent,colAlignment);

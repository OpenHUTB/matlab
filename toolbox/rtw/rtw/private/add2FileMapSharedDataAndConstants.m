function shrdCodeInfo=add2FileMapSharedDataAndConstants(...
    model,...
    sharedutils,...
    objInfoArr,...
    emitCode,...
    ...
    sharedTypesInterface=SharedCodeManager.SharedTypesInterface(sharedFileDmr);
    ...
    try
        ...
        for idx=1:objInfoArr.NumInfo
            ...
            if fileMap.isKey(thisObjName)
                ...
                elseif~checkDataDeclContentsConflict&&~strcmp(oldObj.file,thisObjInfo.file)
                    ...
                    if strcmp(thisObjInfo.kind,'datadecl')||...
                        ...
                        if(~checkDataDeclContentsConflict&&...
                            ~locIsDependentDataTypeShared(thisObjInfo,model,sharedTypesInterface))
            ...
            elseif~checkDataDeclContentsConflict
                ...
                if strcmp(thisObjInfo.kind,'datadecl')||strcmp(thisObjInfo.kind,'datamacro')
                    if~locIsDependentDataTypeShared(thisObjInfo,model,sharedTypesInterface)
                        invalidObjects.MissingType{end+1}={thisObjInfo.name,...
                        thisObjInfo.dependencies{1}};
                ...
                if loc_isUserProvidedType(thisObjInfo)||loc_isUserProvidedData(thisObjInfo)
                    thisObjInfo.doUpdate=false;
                else
        ...
        if(emitCode==1)
            [outFileMap,updateNames,dontUpdateFiles]=locSortBasedOnFile(fileMap,model);
        ...
        if emitCode==1
            ...
            updateNames,useFileRepository,sharedTypesInterface);
    ...
    function siblingAliasTypes=locGetInconsistentlyUsedAliasTypes(oldObjInfo,thisObjInfo)
        ...
        function r=locTypeIsReplacementType(dataTypeName,model)
            ...
            function r=locIsDependentDataTypeShared(thisObjInfo,model,sharedTypesInterface)
                ...
                if(sharedTypesInterface.isValidSharedType(objDataType))
                    return;
                end
                                ...
                                function locDumpSharedCode(modelName,sharedutils,objMap,outFileMap,...
                                    ...
                                    if~isempty(objMap)&&~isempty(updateNames)
                                        ...
                                        for idx=1:length(ofKeys)
                                            ...
                                            [dataMacroVals,dataDeclVals]=locSortObjKind(ofVals{idx});
                                            ...
                                            sortedNames=[sortedDataMacroNames,...
                                            sortedDataDeclNames{:}];
                                            ...
                                            if useFileRepository
                                                ...
                                                if isHeaderFileUsedForUtility(fileName,sharedutils)
                                                    DAStudio.error('RTW:tlc:ErrWhenGenSharedDataConflictModelFile',...
                                                    fileName);
                                                end
                                                ...
                                                if isempty(newFile)
                                                    ...
                                                    if(strcmp(newFile.Group,'data'))
                                                        newFile.OutputDirectory=sharedutils;
                                                    elseif(~newFile.SharedType||~slfeature('AllowSharedDataAndTypesInSameFile'))
                                            ...
                                            for idy=1:numelm
                                                ...
                                                for idz=1:length(depObjNames)
                                                    ...
                                                    if sharedTypesInterface.isValidSharedType(depObjName)
                                                        ...
                                                        if~strcmp(depObj.FileName,ofKeys{idx})
                                                            includedHdrs=locDumpInclude(newFile,depObj.FileName,...
                                            ...
                                            for idy=1:numelm
                                                ...
                                                if useFileRepository
                                                    if(strcmp(aObj.kind,'datamacro'))
                                                        newFile.getDefinesSection().addContent([aObj.definition,newline]);
                                                    elseif(strcmp(aObj.kind,'datadecl'))
                                                        newFile.getDeclarationsSection().addContent([aObj.definition,newline]);
                                                        if slfeature('AllowSharedDataAndTypesInSameFile')&&newFile.SharedType
                                                            newFile.WrittenToDisk=false;
                                                        end
                                                    end
                                                else
                                                    fprintf(newFile,'%s\n\n',aObj.definition);
                                    ...
                                    function[tf]=isHeaderFileUsedForUtility(fileName,sharedutils)




                                        interface=SharedCodeManager.SharedUtilityInterface([sharedutils,'shared_file.dmr']);
                                        lResolvedFiles=coder.internal.xrel.ResolvedFiles(sharedutils);
                                        [srcFiles,lResolveStatus]=getResolveStatus(lResolvedFiles);
                                        fileNameIdx=find(strcmp(srcFiles,fileName)==1);
                                        [~,fileNameNoExtension,~]=fileparts(fileName);
                                        tf=interface.isValidSharedUtility(fileNameNoExtension)&&~isempty(fileNameIdx)&&lResolveStatus(fileNameIdx)~=coder.internal.xrel.FileResolveStatus.Repository




                                        function[outFileMap,updateNames,dontUpdateFiles]=locSortBasedOnFile(objMap,modelName)
                                            ...
                                            for idx=1:length(vals)
                                                ...
                                                vals{idx}.doUpdate=checkUpdateForAliasAndParamInSameHeaderFile(modelName,aFileName,vals{idx}.doUpdate);
                                            ...
                                            function locDumpHeaderGuardBegin(fid,fileName)
                                                ...
                                                function locDumpHeaderGuardEnd(fid,fileName)
                                                    ...
                                                    function updateAgain=checkUpdateForAliasAndParamInSameHeaderFile(modelName,aFileName,currentValue)
                                                        updateAgain=currentValue;
                                                        if~slfeature('AllowSharedDataAndTypesInSameFile')
                                                            return;
                                                        end
                                                        fileRep=get_param(modelName,'SLCGFileRepository');
                                                        if~isempty(fileRep)
                                                            newFile=fileRep.findFileByName(aFileName);
                                                            if~isempty(newFile)&&newFile.SharedType
                                                                updateAgain=true;
                                                            end
                                                        end
                                                            ...
                                                            function locTestForCircularity(objMap,outFileMap)
                                                                ...
                                                                function[fileVec,fileVecSize]=locCheckObjForCircularity(...
                                                                    ...
                                                                    function errMsg=locBuildCircuarityErrorMsg(aDepFile,fileVec,fileVecSize)
                                                                        ...
                                                                        function[dataMacroVals,dataDeclVals]=locSortObjKind(vals)
                                                                            ...
                                                                            function sortedNames=locDoSortOnDataDeclMacro(aList,aObjMap)
                                                                                ...
                                                                                sortedList=struct;
                                                                                objNameMap=containers.Map('KeyType','char','ValueType','any');
                                                                                for k=1:length(aList)
                                                                                    ...
                                                                                    if isfield(aList{k},'isVariantControl')&&aList{k}.isVariantControl
                                                                                    ...
                                                                                    if length(objName)>63
                                                                                        objName=objName(1:62);
                                                                                        if objNameMap.isKey(objName)
                                                                                            objNameMap(objName)=objNameMap(objName)+1;
                                                                                        else
                                                                                            objNameMap(objName)=0;
                                                                                        end
                                                                                        if objNameMap(objName)~=0
                                                                                            objName=strcat(objName(1:62),num2str(objNameMap(objName)));
                                                                                        end

                                                                                    end
                                                                                    sortedList.(pkgName).(msName).(cscName).(isVCtrl).(objName)=aList{k};
                                                                                end
                                                                                ...
                                                                                for pkg_idx=1:length(pkgNames)
                                                                                    ...
                                                                                    for ms_idx=1:length(msNames)
                                                                                        ...
                                                                                        for csc_idx=1:length(cscNames)
                                                                                            ...
                                                                                            vctrls=sort(fieldnames(sortedList.(aPkgName).(aMsName).(aCscName)));
                                                                                            for vctrl_idx=1:length(vctrls)
                                                                                                vctrl=vctrls{vctrl_idx};
                                                                                                objNames=sort(fieldnames(sortedList.(aPkgName).(aMsName).(aCscName).(vctrl)));
                                                                                                for obj_idx=1:length(objNames)
                                                                                                    aObjName=objNames{obj_idx};

                                                                                                    aEleValue=sortedList.(aPkgName).(aMsName).(aCscName).(vctrl).(aObjName);
                                                                                ...
                                                                                function anyChange=locResetDoUpdateFlag(objMap)
                                                                                    ...
                                                                                    function oChecksumStr=locComputeMD5Checksum(value)
                                                                                        ...
                                                                                        value=strtrim(removeTempInlineTraceComments(value));
                                                                                                                        ...
                                                                                                                        function locPragmasChanged(invalidObjectsForPragmas)%#ok
                                                                                                                            ...
                                                                                                                            function locDataScopeChanged(invalidObjectForDataScope)%#ok
                                                                                                                                ...
                                                                                                                                function retVal=loc_isUserProvidedType(objInfo)
                                                                                                                                    ...
                                                                                                                                    function retVal=loc_isUserProvidedData(objInfo)
                                                                                                                                        retVal=false;
                                                                                                                                        if(strcmp(objInfo.kind,'datadecl')&&...
                                                                                                                                            isfield(objInfo,'dataScope')&&...
                                                                                                                                            strcmp(objInfo.dataScope,'Imported'))
                                                                                                                                            retVal=true;
                                                                                                                                        end

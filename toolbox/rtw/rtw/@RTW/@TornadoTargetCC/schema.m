function schema()
    ...
    if isempty(findtype('CONFIG_TARGET_TORNADO_CODEFORMAT_ENUM'))
        schema.EnumType('CONFIG_TARGET_TORNADO_CODEFORMAT_ENUM',...
        {'RealTime','RealTimeMalloc'},[0,1]);
    end
    hThisProp=add_prop(hThisClass,'CodeFormat',...
    'CONFIG_TARGET_TORNADO_CODEFORMAT_ENUM');
    hThisProp.FactoryValue='RealTime';

    hThisProp=add_prop(hThisClass,'StethoScope','on/off');
    hThisProp.FactoryValue='off';

    hThisProp=add_prop(hThisClass,'DownloadToVxWorks','on/off');
    hThisProp.FactoryValue='off';

    hThisProp=add_prop(hThisClass,'BasePriority','slint');
    hThisProp.FactoryValue=30;

    hThisProp=add_prop(hThisClass,'TaskStackSize','slint');
    hThisProp.FactoryValue=16384;
    ...
    hThisProp=add_prop(hThisClass,'RTWCAPIParams','on/off');
    hThisProp.FactoryValue='off';

    hThisProp=add_prop(hThisClass,'RTWCAPISignals','on/off');
    hThisProp.FactoryValue='off';

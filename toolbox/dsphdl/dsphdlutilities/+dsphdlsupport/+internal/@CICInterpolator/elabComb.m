function cSection=elabComb(~,hTopN,blockInfo,slRate,rdyout_re,rdyout_im,rdy_valid,internalReset,combOut_re,combOut_im,c_vout)
    ...
    'Network',hTopN,...
    'Name','cSection',...
    'InportNames',{'rdyout_re','rdyout_im','rdy_valid','internalReset'},...
    ...
    rdyout_re=cSection.PirInputSignals(1);
    rdyout_im=cSection.PirInputSignals(2);
    ...
    combOutreg_re=cSection.addSignal(pir_sfixpt_t(blockInfo.stageDT{blockInfo.NumSections}.WordLength,blockInfo.stageDT{blockInfo.NumSections}.FractionLength),'combOutreg_re');
    combOutreg_re.SimulinkRate=slRate;
    combOutreg_im=cSection.addSignal(pir_sfixpt_t(blockInfo.stageDT{blockInfo.NumSections}.WordLength,blockInfo.stageDT{blockInfo.NumSections}.FractionLength),'combOutreg_im');
    combOutreg_im.SimulinkRate=slRate;


    invalidOut_re=cSection.addSignal(pir_sfixpt_t(blockInfo.stageDT{blockInfo.NumSections}.WordLength,blockInfo.stageDT{blockInfo.NumSections}.FractionLength),'invalidOut_re');
    invalidOut_re.SimulinkRate=slRate;
    invalidOut_im=cSection.addSignal(pir_sfixpt_t(blockInfo.stageDT{blockInfo.NumSections}.WordLength,blockInfo.stageDT{blockInfo.NumSections}.FractionLength),'invalidOut_im');
    ...
    pirelab.getDTCComp(cSection,rdyout_re,cIn_re(1),'Floor','Wrap');
    pirelab.getDTCComp(cSection,rdyout_im,cIn_im(1),'Floor','Wrap');
    pirelab.getWireComp(cSection,rdy_valid,c_vin(1));
    for i=1:blockInfo.NumSections

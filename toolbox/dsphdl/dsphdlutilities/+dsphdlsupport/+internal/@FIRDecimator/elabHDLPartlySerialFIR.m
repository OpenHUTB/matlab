function FIRPartlySerial=elabHDLPartlySerialFIR(this,FilterImpl,blockInfo,insignals,outsignals)
    ...
    FIRPartlySerial=pirelab.createNewNetwork(...
    'Network',FilterImpl,...
    'Name',Name,...
    ...
    FIRFilter=dsphdlsupport.internal.FIRFilter;
    FIRFilter.elaborateSystolicFIRSharing(FIRPartlySerial,...
    FIRPartlySerial.PirInputSignals,FIRPartlySerial.PirOutputSignals,blockInfo);

    for loop=2:length(FilterImpl.PirInputSignals)
        FIRPartlySerial.PirInputSignals(loop).SimulinkRate=FIRPartlySerial.PirInputSignals(1).SimulinkRate;
    end

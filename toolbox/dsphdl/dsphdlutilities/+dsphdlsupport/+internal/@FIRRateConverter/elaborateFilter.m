function newNet=elaborateFilter(this,hN,blockInfo,dataInType,dataOutType,phaseType)
    ...
    'InportNames',{'data','dataValid','phase','phaseValid'},...
    'InportTypes',[dataInType,pir_boolean_t,phaseType,pir_boolean_t],...
    ...
    [accNT,yNT]=dsphdl.FIRRateConverter.getPrecision(...
    ...
    pirelab.getWireComp(newNet,phaseValid,capture);
    ...
    for k=1:delayLineLength
        ...
        pirelab.getIntDelayComp(newNet,coeffReg(k),coeffMultPipe(k),2,...
        ['productPipeInst',kstr]);
        pirelab.getIntDelayComp(newNet,tapReg(k),tapMultPipe(k),2,...
        ['productPipeInst',kstr]);
        ...
        pirelab.getIntDelayComp(newNet,product(k),productPipe(k),2,...
        ['productPipeInst',kstr]);
    ...
    elaborateAdderTree(newNet,adderInput,acc);


    pirelab.getDTCComp(newNet,acc,y,blockInfo.RoundingMethod,blockInfo.OverflowAction);


    pirelab.getUnitDelayComp(newNet,y,dataOut,'outputRegInst');
    ...
    pirelab.getIntDelayComp(newNet,phaseValid,validOut,...
...
function elaborateAdderTree(hN,terms,sumOut)
    ...
    for ks=1:numStages
        ...
        for ka=1:numAdders
            ...
            pirelab.getUnitDelayComp(hN,adderOut,termsOut(ka),...
            [name,'RegInst']);
        ...
        if spareTerm
            ...
            pirelab.getUnitDelayComp(hN,termsIn(numTerms),termsOut(numAdders+1),...
            [name,'RegInst']);

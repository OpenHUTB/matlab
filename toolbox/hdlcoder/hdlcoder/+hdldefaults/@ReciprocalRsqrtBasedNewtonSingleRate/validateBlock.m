function v=validateBlock(~,hC)
    ...
    if(~strcmpi(functionName,'Reciprocal'))
        v(end+1)=hdlvalidatestruct(1,message('hdlcoder:validate:recipSRunsupported',functionName));
    else
        ...
        if hdlarch.newton.isNewtonSqrtOverLimit(hInSignals)
            v(end+1)=hdlvalidatestruct(1,message('hdlcoder:validate:reciprsqrtbasedoverlimit','ReciprocalRsqrtBasedNewtonSingleRate'));
        end
        ...
        if inputWL==3
            v(end+1)=hdlvalidatestruct(1,message('hdlcoder:validate:recipSRminwordlen'));
        end

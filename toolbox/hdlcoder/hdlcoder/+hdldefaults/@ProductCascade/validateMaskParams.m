function vstructs=validateMaskParams(~,hC)
    ...
    if~isempty(strfind(inputsigns,'/'))
        vstructs(end+1)=hdlvalidatestruct(1,message('hdlcoder:validate:ProductCascadeDivide'));
    end


    if length(hC.SLInputPorts)>1
        vstructs(end+1)=hdlvalidatestruct(1,message('hdlcoder:validate:ProductCascadeInputs'));
    end
    ...
    if isequal(inputRate,Inf)
        vstructs(end+1)=hdlvalidatestruct(1,message('hdlcoder:validate:ProductCascadeInfRate'));
    end

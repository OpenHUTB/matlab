function v=validateBlock(this,hC)
    ...
    if(~strcmpi(functionName,'sqrt'))
        v(end+1)=hdlvalidatestruct(1,message('hdlcoder:validate:newtonsqrtSRunsupported',functionName));
    else
        ...
        if hdlarch.newton.isNewtonSqrtOverLimit(hInSignals)
            v(end+1)=hdlvalidatestruct(1,message('hdlcoder:validate:newtonrsqrtSRoverlimit'));
        end
        ...
        if inputWL<4
            v(end+1)=hdlvalidatestruct(1,message('hdlcoder:validate:newtonrsqrtSRunderlimit'));
        end

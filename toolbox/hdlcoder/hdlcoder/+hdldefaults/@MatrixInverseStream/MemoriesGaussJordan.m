...
function MemoriesGaussJordan(~,hN,MemoriesInSigs,MemoriesOutSigs,...
    ...
    'Name','MemoriesGaussJordan',...
    'InportNames',{'wrtEnb','wrtAddr','wrtData','rdAddr'},...
    'InportTypes',[pirelab.createPirArrayType(hBoolT,[blockInfo.MatrixSize*2,0]),...
    ...
    RdDataArray=hdlhandles(blockInfo.MatrixSize*2,1);

    ReadDataS=hdlhandles(blockInfo.MatrixSize*2,1);
    ...
    wrtEnbUfix1S=hdlhandles(blockInfo.MatrixSize*2,1);
    if blockInfo.MatrixSize<=2
        wrtAddrUfix2S=hdlhandles(blockInfo.MatrixSize*2,1);
        rdAddrUfix2S=hdlhandles(blockInfo.MatrixSize*2,1);
    end

    for i=1:blockInfo.MatrixSize*2
    ...
    if(blockInfo.MatrixSize<=2)
        for j=1:blockInfo.MatrixSize*2
            suffix1=['_',int2str(j)];
    ...
    for itr=1:blockInfo.MatrixSize*2

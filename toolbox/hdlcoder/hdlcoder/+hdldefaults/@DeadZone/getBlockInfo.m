function[lowerLimit,upperLimit,rndMode,satMode]=getBlockInfo(~,hC)
    ...
    ul_val=getResolvedInfo(slbh,'UpperValue');
    ll_val=getResolvedInfo(slbh,'LowerValue');

    rndMode='Nearest';
    saturate_on=get_param(slbh,'SaturateOnIntegerOverflow');
    if(any(strcmpi(saturate_on,'on')))
        satMode='saturate';
    else
        satMode='Wrap';
    end

...
function[timing,synarea,metadata]=readQuartusProResultsFromFile(this,stage)
    metadata.dut='';
    metadata.synthTool='';
    metadata.chipName='';
    metadata.deviceName='';
    ...
    if this.readResources
        if strcmpi(stage,'Synthesis')||strcmpi(stage,'Map')
            ...
            [metadata.RPTFileName,metadata.RPTFileTimestamp]=systemFindUnique(this,[this.dutName,'_qpro.syn.rpt'],'Altera Analysis & Synthesis Report file','Please ensure that you have run "Perform Logic Synthesis" step in the HDL Workflow Advisor before parsing the results.');

            fh0=fopen(metadata.RPTFileName,'r');
            fh1=fopen([this.dutName,'_qpro.fit.rpt'],'r');
            ...
            dutE1='Fitter\s+report\s+for\s+(\w+)_qpro';

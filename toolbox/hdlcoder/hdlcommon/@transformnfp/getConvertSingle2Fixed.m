function hTopN=getConvertSingle2Fixed(hN,latency,slRate,WL,UWL,FL,rndMode,...
    satMode,flGtZero)
    ...
    p=pir(hN.getCtxName);
    topNwkName='nfp_convert_single_to_';
    if WL==UWL
...
function createNetworks(p,topNwkName,pipestage,slRate,WL,UWL,FL,rndMode,...
    satMode,flGtZero)
    if FL<24
        guard=24-FL;
    else
    ...
    if~strcmp(rndMode,'Ceiling')&&~strcmp(rndMode,'Floor')
        createNetwork_n5(p,hN_n5,pipestage,slRate,WL,UWL,FL,guard);
    else
        createNetwork_n5_ceil(p,hN_n5,pipestage,slRate,WL,UWL,FL,guard);
    end
...
function hN=createNetwork_n5(~,hN,pipestage,slRate1,WL,~,FL,guard)
    ...
    pirelab.getMinMaxComp(hN,...
    [Delay1_out1_s5,shiftArithmetic2_zerosig_s17],...
...
function hN=createNetwork_n5_ceil(~,hN,pipestage,slRate1,WL,~,FL,guard)
    pirTyp4=pir_boolean_t;
    pirTyp1=pir_sfixpt_t(9,0);
    pirTyp2=pir_ufixpt_t(24,-23);
    pirTyp3=pir_ufixpt_t((WL+guard),-(FL+guard));
    ...
    pirelab.getMinMaxComp(hN,...
    [Delay1_out1_s8,shiftArithmetic2_zerosig_s23],...
...
function hN=createNetwork_n1(p,hN,topNwkName,pipestage,slRate1,...
    ...
    nt2=numerictype(0,24,23);
    nt5=numerictype(0,(WL+guard),(FL+guard));
    if WL==1
    ...
    Delay_out1_s4=addSignal(hN,'Delay_out1',pirTyp5,slRate1);
    Delay1_out1_s5=addSignal(hN,'Delay1_out1',pirTyp6,slRate1);
    ...
    Delay9_out1_s15=addSignal(hN,'Delay9_out1',pirTyp7,slRate1);
    ...
    [Delay9_out1_s15,Delay10_out1_s6],...
    AddSign_out1_s3);

function hC=getExpMegaFunctionComp(targetCompInventory,hN,hInSignals,hOutSignals,className,mnemonic,pipeline)
    ...
    fprintf(fid,'ROUNDING=TO_NEAREST\n');
    fprintf(fid,'WIDTH_EXP=%d\n',WIDTH_EXP);
    fprintf(fid,'WIDTH_MAN=%d\n',WIDTH_MAN);


    alteratarget.applyExtraArgs(fid,'Exp',baseType);
    ...
    if~isempty(targetCompInventory)
        ...
        alteratarget.generateMegafunction(targetCompInventory,altMegaFunctionName,'altfp_exp',mfparamsTempFile,pipeline,num);
    end

classdef DDGWebKitWindow<handle
    ...
    methods(Access=private)
        function self=DDGWebKitWindow(title,url,varargin)
            p=inputParser();
            p.FunctionName='sltest.testsequence.internal.view.DDGWebKitWindow';
            ...
            self.browser.Tag='webbrowser';
            self.browser.Type='webbrowser';
            self.browser.WebKit=true;
    ...
    methods(Static)
        function dlg=create(title,url,varargin)
            dlg=DAStudio.Dialog(sltest.testsequence.internal.view.DDGWebKitWindow(title,url,varargin{:}));
        end
    end
end

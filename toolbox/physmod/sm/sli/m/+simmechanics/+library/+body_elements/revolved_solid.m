function varargout=revolved_solid(varargin)
    ...
    if isempty(BlockInfoCache)
        blockReader=simmechanics.library.helper.BlockXmlReader('revolvedSolid');
        BlockInfoCache=blockReader.generateBlockInfo(mfilename('fullpath'),...
        pm_message('sm:library:bodyElements:revolvedSolid:Name'));
    end

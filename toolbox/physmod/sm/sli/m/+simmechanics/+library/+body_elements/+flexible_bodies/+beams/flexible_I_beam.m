function varargout=flexible_I_beam(varargin)
    ...
    if isempty(BlockInfoCache)
        blockReader=simmechanics.library.helper.BlockXmlReader('flexibleIBeam');
        srcFile=mfilename('fullpath');
        blkName=pm_message('sm:library:bodyElements:flexibleBodies:beams:flexibleIBeam:Name');

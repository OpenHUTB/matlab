function varargout=flexible_cylindrical_beam(varargin)
    ...
    if isempty(BlockInfoCache)
        blockReader=simmechanics.library.helper.BlockXmlReader('flexibleCylindricalBeam');
        srcFile=mfilename('fullpath');
        blkName=pm_message('sm:library:bodyElements:flexibleBodies:beams:flexibleCylindricalBeam:Name');

function varargout=flexible_channel_beam(varargin)
    ...
    if isempty(BlockInfoCache)
        blockReader=simmechanics.library.helper.BlockXmlReader('flexibleChannelBeam');
        srcFile=mfilename('fullpath');
        blkName=pm_message('sm:library:bodyElements:flexibleBodies:beams:flexibleChannelBeam:Name');

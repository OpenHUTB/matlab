function varargout=spring_damper_force(varargin)
    ...
    if isempty(BlockInfoCache)
        ...
        pm_message('sm:library:forcesAndTorques:springDamper:Name');
        ...
        pm_message('sm:model:blockNames:springDamperForce:TypeId'));

        maskParams=[maskParams,make_params(...
        msgFcn(fullId('naturalLength')),...
        '0',...
        msgFcn(fullId('naturalLengthUnits')),...
        'm',true)];

        maskParams=[maskParams,make_params(...
        msgFcn(fullId('springStiffness')),...
        '0',...
        msgFcn(fullId('springStiffnessUnits')),...
        'N/m',true)];

        maskParams=[maskParams,make_params(...
        msgFcn(fullId('dampingCoefficient')),...
        '0',...
        msgFcn(fullId('dampingCoefficientUnits')),...
        'N/(m/s)',true)];
...
function fullMsgId=fullId(msgId)
    fullMsgId=['mech2:messages:parameters:force:springDamperForce:',msgId,':ParamName'];
end

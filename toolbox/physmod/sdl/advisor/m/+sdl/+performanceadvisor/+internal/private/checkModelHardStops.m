function obj=checkModelHardStops(objType)
...
function result=checkCallback(system)
    ...
    if isempty(blocks)
        ...
        ModelHardStopStr=get_param(blocks,'model_hardstop');

        isDisabled=strcmp('0',ModelHardStopStr);


        menuStrings={getMessage('ModelHardStopsNo'),getMessage('ModelHardStopsYes')};
        for blockIdx=1:length(blocks)
            thisBlock=blocks{blockIdx};
            thisEnabledStr=menuStrings{1+str2double(ModelHardStopStr(blockIdx))};
...
function result=updateAction(taskObj)
    ...
    for i=1:numel(blocks)
        thisBlock=blocks{i};
        try
            set_param(thisBlock,'model_hardstop','0');
        catch

function obj=checkTorqueConverterLag(objType)
...
function result=checkCallback(system)
    ...
    if isempty(blocks)
        ...
        TorqueConverterLagStr=get_param(blocks,'model_lag');

        isDisabled=strcmp('0',TorqueConverterLagStr);


        menuStrings={getMessage('TorqueConverterLagNo'),getMessage('TorqueConverterLagYes')};
        for blockIdx=1:length(blocks)
            thisBlock=blocks{blockIdx};
            thisEnabledStr=menuStrings{1+str2double(TorqueConverterLagStr(blockIdx))};
...
function result=updateAction(taskObj)
    ...
    for i=1:numel(blocks)
        thisBlock=blocks{i};
        try
            set_param(thisBlock,'model_lag','0');
        catch

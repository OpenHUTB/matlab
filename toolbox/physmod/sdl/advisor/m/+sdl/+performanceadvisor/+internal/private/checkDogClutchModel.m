function obj=checkDogClutchModel(objType)
...
function result=checkCallback(system)
    ...
    if isempty(blocks)
        ...
        DogClutchModelStr=get_param(blocks,'model');

        isFrictionClutch=strcmp('1',DogClutchModelStr);


        menuStrings={getMessage('FrictionClutchYes'),getMessage('FrictionClutchNo')};
        for blockIdx=1:length(blocks)
            thisBlock=blocks{blockIdx};
            thisDisabledStr=menuStrings{str2double(DogClutchModelStr(blockIdx))};
            ft.addRow({thisBlock,thisDisabledStr});
        end

        if all(isFrictionClutch)
...
function blocks=findSuboptimalBlocks(system)
    if Simulink.internal.useFindSystemVariantsMatchFilter()
        ...
        'MaskType','(Dog Clutch|Synchronizer)',...
    ...
    models=get_param(blocks,'model');
    idx=[];
    for i=1:length(models)
        temp=strfind(models{i},'1');
        if isempty(temp)
            idx=[idx,i];%#ok<AGROW>
        end
    end

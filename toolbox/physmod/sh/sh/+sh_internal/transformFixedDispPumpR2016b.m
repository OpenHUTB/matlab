function[outData]=transformFixedDispPumpR2016b(inData)
    ...
    if(ismember('efficiency_tot',parameterNames))...
        ...
        displacement_index=strncmp('D',parameterNames,2);
        displacement=instanceData(displacement_index).Value;
        displacement_unit_index=strcmp('D_unit',parameterNames);
        displacement_unit=instanceData(displacement_unit_index).Value;


        instanceData(end+1).Name='torque_pressure_coeff';
        instanceData(end).Value=[num2str(value(simscape.Value(1,displacement_unit),'m^3/rad')),'*(',displacement,')*((',volume_efficiency,')/(',total_efficiency,')-1 )'];

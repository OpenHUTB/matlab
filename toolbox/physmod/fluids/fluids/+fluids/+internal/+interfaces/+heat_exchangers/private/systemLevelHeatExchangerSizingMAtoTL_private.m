...
function[geometry_TL,geometry_MA,Q_seg_calc,T_nodes_TL,rho_avg_TL,...
    ...
    =systemLevelHeatExchangerSizingMAtoTL_private(flow_arrangement,Q_nom,...
...
%#codegen
    ...
    if T_in_MA<=T_in_TL
        exitflag=-10;
        return
    elseif T_seg_MA<=T_seg_TL
        exitflag=-20;
        return
    end


    UA_overall=Q_nom/(T_seg_MA-T_seg_TL);
    ...
    if flow_arrangement==1
        ...
        [~,Q_seg_calc(:,1),T_nodes_TL_tmp,hbar_nodes_MA_tmp,HR_nodes_MA_tmp,...
    ...
    elseif flow_arrangement==2
        ...
        [~,Q_seg_calc,T_nodes_TL,hbar_nodes_MA,HR_nodes_MA,exitflag_inner]=fcn(xG);
    end
    ...
    if flow_arrangement==1
        if T_out_avg_MA<T_out_TL
            exitflag=-13;
        end
    else
        if T_out_avg_MA<T_in_TL
            exitflag=-12;
        end
    end
...
function[residual,Q_seg_calc,T_TL,hbar_MA,HR_MA,exitflag]=heatTransferColinear(...
    xG,N,Q_nom,...
    geometry_TL_init,h_in_TL,T_in_TL,mu_in_TL,k_in_TL,Pr_in_TL,props_TL,...
    geometry_MA_init,hbar_in_MA,HR_in_MA,props_MA,...
    opt_inner_search,opt_inner_solve)
...
function[residual,Q_seg_calc,T_TL,hbar_MA,HR_MA,exitflag]=heatTransferGrid(...
    xG,N,Q_nom,...
    geometry_TL_init,h_in_TL,T_in_TL,mu_in_TL,k_in_TL,Pr_in_TL,props_TL,...
    geometry_MA_init,hbar_in_MA,HR_in_MA,props_MA,...
    opt_inner_search,opt_inner_solve)

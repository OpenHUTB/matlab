function out=update_pump_var_displ(hBlock)
    ...
    if strcmp(SourceFile,'sh.pumps_motors.pump_var_displ')
        port_names={'C','S','P','T'};
    elseif strcmp(SourceFile,'sh.pumps_motors.variable_displacement_pump_input_efficiency')
        port_names={'D','EV','EM','S','P','T'};
    else
        port_names={'D','LV','LM','S','P','T'};
    end
    ...
    HtoIL_set_block_files(hBlock,'SimscapeFluids_lib/Isothermal Liquid/Pumps & Motors/Variable-Displacement Pump (IL)')
    ...
    if strcmp(SourceFile,'sh.pumps_motors.pump_var_displ')
        connections.destination_ports=HtoIL_collect_destination_ports(hBlock,[1,3,2,5]);
    elseif strcmp(SourceFile,'sh.pumps_motors.variable_displacement_pump_input_efficiency')

        set_param(hBlock,'leakage_friction_spec','fluids.isothermal_liquid.pumps_motors.enum.pump_motor_loss_spec.input_efficiency');

        connections.destination_ports=HtoIL_collect_destination_ports(hBlock,[1,2,3,5,4,7]);
    else
    ...
    if strcmp(SourceFile,'sh.pumps_motors.pump_var_displ')
        ...
        if strcmp(modelType,'1')
            ...
            pump_ports=get_param(hBlock,'PortHandles');
            pump_D_port=pump_ports.LConn(1);
        ...
        add_line(connections.subsystem,ps_sat_port_out,pump_D_port);
        connections.destination_ports(1)=ps_port_in;
    end


    if strcmp(SourceFile,'sh.pumps_motors.pump_var_displ')&&strcmp(lossSpec,'1')
        warnings.messages{1,1}='Nominal fluid density and kinematic viscosity removed. Pump uses network fluid properties. Adjustment of Volumetric efficiency at nominal conditions may be required.';

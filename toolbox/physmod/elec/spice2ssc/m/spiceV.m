    ...
    methods
        function this=spiceV(str)
            ...
            if nargin<1
                ...
                str=string(str);
                if length(str)>1
                    pm_error('physmod:ee:library:TooMany',getString(message('physmod:ee:library:comments:spice2ssc:spiceV:error_InputStringsToSpiceV')));
                end
                ...
                if~strncmpi(this.name,this.id,1)
                    pm_warning('physmod:ee:spice2ssc:UnexpectedComponentIdentifier',this.id,getString(message('physmod:ee:library:comments:spice2ssc:spiceV:warning_VoltageSource')),this.name);
                end
                ...
                if tag_index~=0
                    if any(cellfun(@length,strComponents(idx(tag_index):end))>1)
                        pm_error('physmod:ee:spice2ssc:UnsupportedFormat',this.name,getString(message('physmod:ee:library:comments:spice2ssc:spiceV:error_TransientSpecificationMustOccurAfterAllnamevaluePairs')));
                    end
                ...
                case this.iexp
                    ...
                    this.value=this.value+")";
                case this.isin
                    this.value="ee.additional.spice_sources.volt_sin(";
                    usedIndices=[usedIndices,idx(tag_index)+(0:min([6,length(fnArgElements)]))];
                    for ii=1:min([6,length(fnArgElements)])
                        switch ii
                        case 1
                            this.value=this.value...
                            +"VO={"+fnArgElements(ii)+",'V'}";
                        case 2
                            this.value=this.value...
                            +"VA={"+fnArgElements(ii)+",'V'}";
                        case 3
                            this.value=this.value...
                    ...
                    this.value=this.value+")";
                case this.ipwl
                    this.value="ee.additional.spice_sources.volt_pwl(";
                    ...
                    +"],'s'},VOLTlist={["+strjoin(sig_vect)...
                    +"],'V'})";
                case this.ipulse
                    this.value="ee.additional.spice_sources.volt_pulse(";
                    ...
                    this.value=this.value+")";
                case this.isffm
                    this.value="ee.additional.spice_sources.volt_sffm(";

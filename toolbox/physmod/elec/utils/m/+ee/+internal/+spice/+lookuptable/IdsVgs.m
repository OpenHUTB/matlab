function[VgsVec,VdsVec,TVec,IdsMat]=IdsVgs(netlistOut,subcircuitName)
    ...
    IdsVgs=regexp(name,'(?<=\_)IdsVgs(?=\_)','match');
    name=name(~cellfun(@isempty,IdsVgs));
    netlistOutPath=netlistOutPath(~cellfun(@isempty,IdsVgs));

    if isempty(name)

        pm_error('physmod:ee:utilities:spice:semiconductorSubcircuit2lookup:FormatErrorInSPICEOutputName',...
        getString(message('physmod:ee:utilities:spice:semiconductorSubcircuit2lookup:NoFileNameMatchForIdsVgs','_IdsVgs')));
    else
    ...
    for ii=1:length(netlistOutPath)
        ...
        [VgsLength,~]=size(analysis.variableVectors{1});
        IdsCell{ii}=zeros(VgsLength,length(VdsCell{ii}));
        flagT=0;
        ...
        for jj=1:length(VdsCell{ii})
            ...
            if length(VgsCell{1})==length(Vgs)
                if~all(VgsCell{1}==Vgs)
            ...
            if~all(all(round(Vds-VdsCell{ii}(jj),4)==0))
    ...
    for ii=2:length(TVec)
        ...
        if~all(VdsCell{ii}==VdsCell{1})

            pm_error('physmod:ee:utilities:spice:semiconductorSubcircuit2lookup:FormatErrorInSPICEOutput',...
            getString(message('physmod:ee:utilities:spice:semiconductorSubcircuit2lookup:TheValueOfShouldBeTheSame','Vds','IV characteristics')));
        end

        for jj=1:length(VdsCell{1})

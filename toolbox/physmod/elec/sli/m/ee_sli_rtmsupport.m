function varargout=ee_sli_rtmsupport(event,handle)
    ...
    otherwise
        pm_abort(getString(message('physmod:ee:library:comments:utils:pmInternalError:error_UnknownCallback')));
    end
    ...
    function editModes=l_get_parameter_editing_modes(handle,updateMaskEnables)
        ...
        editModes=ee_setparammode(authoringParams(abs(restrictedParameterList)));
        ...
        elseif~pmsl_checklicense('Power_System_Blocks')
            modelInRestrictedMode=true;
        end
        ...
        function rtmAuthoringParams=ee_setparammode(authoringModeParams)

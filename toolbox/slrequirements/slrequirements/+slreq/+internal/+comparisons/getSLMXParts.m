function parts=getSLMXParts(filePath)
    ...
    try
        ...
        if(strcmp(ME.identifier,'Simulink:LoadSave:InvalidModelPackage'))
            msg=getString(message('Slvnv:slreq:InvalidCorruptSLMXFile',filePath));
            msg=strrep(msg,'\','\\');
            ME=MException('Slvnv:slreq:InvalidCorruptSLMXFile',msg);
            throw(ME);
        else
            rethrow(ME);
        end

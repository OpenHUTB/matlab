...
classdef LabelConfiguration<matlab.mixin.Copyable
...
LabelColors
    ...
    methods
        ...
        function obj=LabelConfiguration(volDataL,uniqueLabels,uniqueLabelsScaled)

            obj.RenderingStyle='LabelVolumeRendering';
            obj.Lighting=true;

            if strcmp(volDataL,'default')
                obj.Labels=1:254;
                obj.NumLabels=length(obj.Labels);
                obj.MaxLabel=obj.Labels(end);
                obj.LabelNames=generateLabelNames(obj.Labels);
                obj.LabelsPresentIdx=[];

            else
                if iscategorical(volDataL)
                    cats=categories(volDataL);
                    if any(isundefined(volDataL(:)))
                        obj.LabelNames=["000";string(cats)];
                        obj.NumLabels=length(obj.LabelNames);
                        obj.OriginalLabels=0:obj.NumLabels-1;
                        obj.Labels=1:obj.NumLabels;
                    else
                        obj.LabelNames=string(cats);
                        obj.NumLabels=length(obj.LabelNames);
                        obj.OriginalLabels=1:obj.NumLabels;
                        obj.Labels=2:obj.NumLabels+1;
                    end
                    maxLabelLength=10;
                    obj.LabelNames=arrayfun(@(x)extractBefore(x,min(strlength(x)+1,maxLabelLength)),obj.LabelNames);
                    obj.LabelsPresentIdx=1:obj.NumLabels;
                    obj.MaxLabel=obj.Labels(end);

                else
                    obj.OriginalLabels=uniqueLabels;
                    obj.Labels=uniqueLabelsScaled;
                    obj.MaxLabel=obj.Labels(end);
                    obj.NumLabels=numel(obj.OriginalLabels);
                    obj.LabelNames=generateLabelNames(obj.OriginalLabels);
                    obj.LabelsPresentIdx=1:obj.NumLabels;
                end
            end

            obj.NumLabels=double(obj.NumLabels);
            if obj.NumLabels>254
                ME=MException('images:invalidFile',getString(message('images:volumeViewer:numLabelsExceeded')));
                throw(ME);
            end

            obj.MaxLabel=double(obj.MaxLabel);
            obj.setRenderingDefaults();

        end


        function setRenderingDefaults(self)

            self.ShowFlags=false(self.NumLabels,1);
            self.ShowFlags(self.LabelsPresentIdx)=true;

            self.ShowLabelThumbs=zeros(1,self.NumLabels);
            self.ShowLabelThumbs(self.ShowFlags)=self.LabelsPresentIdx;

            self.Opacities=ones(self.NumLabels,1);

            if self.OriginalLabels(1)==0
                self.ShowFlags(1)=false;
                self.Opacities(1)=0;
            end

            cmap=getDefaultColormap;
            self.Colormap=cmap;
        end

        function idx=getLabelIdxFromBrowserSelection(self,idxBrowser)
            label=self.Labels(idxBrowser);
            idx=label+1;
        end
    ...
    methods
        ...
        function set.Colormap(self,cmap)
            validateattributes(cmap,{'numeric'},{'2d','finite','nonnegative','nonsparse'});
            assert(size(cmap,2)==3,'Colormap must have 3 columns');
            self.Colormap=cmap;
        end
        ...
        function amap=get.Alphamap(self)
            ...
            for i=1:self.NumLabels
                label=self.Labels(i);
                labelIdx=label+1;
                amap(labelIdx)=self.ShowFlags(i).*self.Opacities(i);
            end
        end

        function labelColors=get.LabelColors(self)
            labelColors=self.Colormap(self.Labels+1,:);
        end

        function originalLabels=get.OriginalLabels(self)
            if isempty(self.OriginalLabels)
                originalLabels=self.Labels;
            else
                originalLabels=self.OriginalLabels;
            end
        end
    end
end
...
function names=generateLabelNames(labels)
    names=labels;
    names=string(arrayfun(@(x)sprintf('Label_%03d',x),names,'UniformOutput',false));
end

function cmap=getDefaultColormap()
    cmap=[

classdef ColormapDesigner<handle
...
Enable
    ...
    methods
        ...
        function self=ColormapDesigner(hPanel,colorControlPoints)
            ...
            self.ColormapList=images.internal.app.volviewToolgroup.MapListManager('volumeColormap');

            self.ColormapLabel=uicontrol(hPanel,'Style','text',...
            'String',getString(message('images:volumeViewerToolgroup:colormap')),...
            'FontWeight','bold',...
            'Units','Normalized');
            self.ColormapLabel.Position=[0.0343,0.85,0.3,0.15];
            self.ColormapLabel.FontSize=self.ColormapLabel.FontSize+1;

            self.ColormapButtonGroup=uibuttongroup(hPanel,...
            'BorderType','none',...
            'Units','normalized',...
            'SelectionChangedFcn',@self.colormapSourceChange);

            self.ColormapButtonGroup.Position=[0,0.4,0.55,0.45];

            self.BuiltinRadioBtn=uicontrol(self.ColormapButtonGroup,...
            'Style','radiobutton',...
            'Tag','cmapFcnButton',...
            'Units','normalized',...
            'Position',[0.05,0.5,1,0.5],...
            'String',getString(message('images:volumeViewerToolgroup:builtinColormap')),...
            'TooltipString',getString(message('images:volumeViewerToolgroup:builtinColormapTooltip')));

            self.WorkspaceRadioBtn=uicontrol(self.ColormapButtonGroup,...
            'Style','radiobutton',...
            'Tag','cmapVarButton',...
            'Units','normalized',...
            'Position',[0.05,0,1,0.5],...
            'String',getString(message('images:volumeViewerToolgroup:wkspVariables')),...
            'TooltipString',getString(message('images:volumeViewerToolgroup:workspaceColormapTooltip')));

            self.BuiltinPopup=uicontrol(hPanel,...
            'Style','popup',...
            'Tag','BuiltinPopup',...
            'String',self.ColormapList.List,...
            'Units','normalized',...
            'Callback',@self.setColormapFromPopup,...
            'TooltipString',getString(message('images:volumeViewerToolgroup:builtinColormapTooltip')));
            self.BuiltinPopup.Position=[0.6,0.65,0.35,0.1667];
            self.BuiltinPopup.Value=self.ColormapList.getDefaultIdx;

            self.WorkspacePopup=uicontrol(hPanel,...
            'Style','popup',...
            'Tag','WorkspacePopup',...
            'String',{''},...
            'Units','normalized',...
            ...
            'Enable','off',...
            'TooltipString',getString(message('images:volumeViewerToolgroup:workspaceColormapTooltip')));
            self.WorkspacePopup.Position=[0.6,0.43,0.35,0.1667];

            self.hAx=axes('Parent',hPanel,'Visible','off','Clipping','off','Tag','Colorbar');
        ...
        function set.Enable(self,TF)
            ...
            if TF
                ...
                if self.BuiltinRadioBtn.Value==1
                    ...
                    if~isempty(self.WorkspacePopup.String)
        ...
        function reactToColormapEdit(self)
            ...
            colormapList=self.BuiltinPopup.String;
            if~strcmp(colormapList{end},getString(message('images:volumeViewerToolgroup:custom')))
                colormapList{end+1}=getString(message('images:volumeViewerToolgroup:custom'));
            end
            self.BuiltinPopup.String=colormapList;
        ...
        function colormapSourceChange(self,~,event)
            ...
            if isempty(self.BuiltinPopup.String{end})
                self.BuiltinPopup.String=self.ColormapList.List;
            ...
            if isequal(event.NewValue,self.BuiltinRadioBtn)
                ...
                colormaps=self.BuiltinPopup.String;
                ...
                if isempty(varList)
                    self.WorkspacePopup.String={'NA'};
                    self.WorkspacePopup.Value=1;
                    self.WorkspacePopup.Enable='off';
                else
                    previousCmap=self.WorkspacePopup.String{self.WorkspacePopup.Value};
                    if any(strcmp(previousCmap,varList))
                        self.WorkspacePopup.String=varList;
                        idx=find(strcmp(previousCmap,self.WorkspacePopup.String));
                        self.WorkspacePopup.Value=idx;
                        self.setNewColormap('WorkspacePopup',previousCmap);
                    else
                        self.WorkspacePopup.String=varList;
                        self.WorkspacePopup.Value=1;
                        self.setNewColormap('WorkspacePopup',self.WorkspacePopup.String{1});
                    end
        ...
        function setNewColormap(self,sourcePopup,newColormapName)
            ...
            case 'BuiltinPopup'
                if strcmp(newColormapName,getString(message('images:volumeViewerToolgroup:custom')))
                    return
                else
                    colormaps=self.BuiltinPopup.String;
                    if strcmp(colormaps{end},getString(message('images:volumeViewerToolgroup:custom')))
                        self.BuiltinPopup.String=colormaps(1:end-1);
                    end
                end

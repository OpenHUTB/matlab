function distMat=bwdistChessboardGPUImpl(img)%#codegen
            ...
            for r=1:rowRepeatVal:h
                ...
                for k=1:maxLookDist
                    ...
                    if(topPixRowIdx>0)
                        ...
                        if(abs(topPixRowIdx-r)<tDist)
                            currDist=single(tDist);
                        else
                            currDist=single(abs(topPixRowIdx-r));
                        end
                    ...
                    if(bottomPixRowIdx<=h)
                        ...
                        if(abs(bottomPixRowIdx-r)<tDist)
                            currDist=single(tDist);
                        else
                            currDist=single(abs(bottomPixRowIdx-r));
                        end
    ...
    coder.gpu.internal.kernelImpl(false,numBlocks,threadsPerBlock,1,'Z_kernel');
    for j=1:w
        for i=1:h
            for k1=1:ch
                minVal=distMat_2D(i,j,k1);
                for k2=1:ch
                    val=max(distMat_2D(i,j,k2),abs(k1-k2));

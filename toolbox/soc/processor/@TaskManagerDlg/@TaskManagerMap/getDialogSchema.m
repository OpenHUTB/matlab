function dialog=getDialogSchema(h,~)
    ...
    dialog.PreApplyMethod='taskManagerMapPreApplyCallback';
    dialog.PreApplyArgs={'%dialog'};
    dialog.PreApplyArgsDT={'handle'};
    dialog.HelpMethod='soc.internal.helpview';
    dialog.HelpArgs={'soc_taskmapping'};
    ...
    dialog.DialogTitle=DAStudio.message('codertarget:utils:TaskMapDlgTitle');
...
function txt=locConstructInfoText(h,rowSpan,colSpan)
    descr=DAStudio.message('codertarget:utils:TaskMapDesc');
...
function tbl=locConstructTaskMappingTable(h)
    ...
    autoAssigned=DAStudio.message('codertarget:utils:AutoAssigned');
    for i=1:nTasks
    ...
    tip=DAStudio.message('codertarget:utils:TaskMapTip');
...
function btn=locConstructTaskMapAutoMapButton(h)
    ...
    rowSpan=[1,1];
    colSpan=[2,2];
    lbl=DAStudio.message('codertarget:utils:TaskMapAutoMapLbl');
    tip=DAStudio.message('codertarget:utils:TaskMapAutoMapTip');
...
function btn=locConstructTaskMapCheckMapButton(h)
    ...
    rowSpan=[2,2];
    colSpan=[2,2];
    lbl=DAStudio.message('codertarget:utils:TaskMapCheckMapLbl');
    tip=DAStudio.message('codertarget:utils:TaskMapCheckMapTip');
    enb=locHasEventDrivenTasks(h);

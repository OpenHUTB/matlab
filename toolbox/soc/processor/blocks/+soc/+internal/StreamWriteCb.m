function varargout = StreamWriteCb( func, blkH, varargin )
...
try 
hAXI4StreamWrite = [ blkPath, '/Variant/CODEGEN/AXI4-Stream Write' ];
...
set_param( hAXI4StreamWrite, 'devName', blkP.DeviceName );
set_param( hAXI4StreamWrite, 'NumBuffers', num2str( blkP.NumberOfBuffers ) );
set_param( hAXI4StreamWrite, 'DataTimeout', dataTimeout );
set_param( hAXI4StreamWrite, 'HideEventLines', hideEventLines );
end 

set_param( hHWSWMessageSend, 'QueueLength', num2str( blkP.NumberOfBuffers ) );

l_SetMaskDisplay( blkH, blkP );
soc.internal.setBlockIcon( blkH, 'socicons.StreamWrite' );
catch ME
hadError = true;
rethrow( ME );
end 
...
helpcmd = 'eval(''soc.internal.helpview(''''soc_streamwrite'''')'')';
set_param( blkH, 'MaskHelp', helpcmd );
end 
% Decoded using De-pcode utility v1.2 from file /tmp/tmpfopRU3.p.

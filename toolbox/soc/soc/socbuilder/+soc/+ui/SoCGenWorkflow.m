classdef SoCGenWorkflow<matlab.hwmgr.internal.hwsetup.Workflow
...
ExtModelInfo
    ...
    methods
        function this=SoCGenWorkflow(varargin)

            if~builtin('license','checkout','SoC_Blockset')
                error(message('soc:utils:NoLicense'));
            end
            this@matlab.hwmgr.internal.hwsetup.Workflow();
            ...
            processorModel=soc.ui.SoCGenWorkflow.getESWRefModel(sys);


            if this.isModelProcessorOnly(sys)
                fpgaModelBlock=[];
                fpgaModel=[];
            else
            ...
            if soc.ui.SoCGenWorkflow.isModelProcessorOnly(sys)
                ...
                this.ExtModelInfo=containers.Map;
                for i=1:numel(CPUWithProcModels)
                    ...
                    this.ExtModelInfo(ExtMdlInfoNew.CPU)=ExtMdlInfoNew;
                end
            end

            this.CancelConfirmationDlg='on';

            this.Name=' ';
            this.Window.Title='SoC Builder';

            this.FirstScreenID='soc.ui.ModelInfo';
            this.sys=sys;
            this.FPGAModel=fpgaModel;
            this.ProcessorModel=processorModel;
            this.ModelType=modelType;
            this.boardName=get_param(sys,'HardwareBoard');


            tskMgrBlks=soc.internal.connectivity.getTaskManagerBlock(...
            this.sys,'overrideAssert');
            if~isempty(tskMgrBlks)
                if~iscell(tskMgrBlks),tskMgrBlks={tskMgrBlks};end
                this.HasEventDrivenTasks=...
                any(cellfun(@(x)soc.internal.taskmanager.hasEventDrivenTasks(x),tskMgrBlks));
            end


            if~this.isRequiredSupportPackageInstalled()
                this.FirstScreenID='soc.ui.HSPInfo';
                return;
            end
            ...
            this.SupportsPeripherals=~isempty(appModel.SupportedPeripheralInfo)&&...
            appModel.arePeripheralBlocksInRefModels();

            [this.HasReferenceDesign,this.ReferenceDesignInfo]=soc.internal.getReferenceDesignInfo(fpgaModel);

            if hasProcessor||...
                (this.HasReferenceDesign&&this.ReferenceDesignInfo.HasProcessingSystem)
                this.SysDeployer=soc.SystemDeployer(sys,'ConnectHardware',false);
                if~this.isModelProcessorOnly(this.sys)
                    this.HWInterfaceObj=soc.internal.oscustomization.HWInterface.getInstance();
                end
            end
            this.BoardSupportsMultipleConfigurations=...
    ...
    methods(Hidden)
        ...
        function UnusedSwCpuMdls=generateModelsForUnusedCPUs(this)
            UnusedCPUs=codertarget.utils.getRegisteredCPUs(this.sys);
            procMdls=this.ProcessorModel;
            if~iscell(procMdls)
                procMdls={procMdls};
            end
            ...
            for i=1:numel(UnusedCPUs)
                ...
                save_system('soc_unused_cpu_top',fullfile(this.socsysinfo.projectinfo.prj_dir,[topMdl,'.slx']));
                save_system('soc_unused_cpu_ref',fullfile(this.socsysinfo.projectinfo.prj_dir,[refMdl,'.slx']));


                load_system('soc_unused_cpu_top');
                load_system('soc_unused_cpu_ref');
                disp(message('soc:utils:SystemModel',fullfile(this.socsysinfo.projectinfo.prj_dir,[topMdl,'.slx'])).getString());
                ...
                textTopAlign=50;
                textLeftAlign=50;
                genText=sprintf('Software executable model generated from %s by SoC Builder on %s',this.sys,datestr(now));
                ...
                hwBoard=get_param(this.sys,'HardwareBoard');
                hwBoardFeatureSet=get_param(this.sys,'HardwareBoardFeatureSet');
                ...
                if iscell(this.socsysinfo.projectinfo.sw_system)
                    this.socsysinfo.projectinfo.sw_system{end+1}=fullfile(this.socsysinfo.projectinfo.prj_dir,[topMdl,'.slx']);
                else
                    this.socsysinfo.projectinfo.sw_system=[this.socsysinfo.projectinfo.sw_system,{fullfile(this.socsysinfo.projectinfo.prj_dir,[topMdl,'.slx'])}];
                end

                UnusedSwCpuMdls{i}=fullfile(this.socsysinfo.projectinfo.prj_dir,[topMdl,'.slx']);
                clear('topswclose');
                clear('refswclose');
            end
        end
        ...
        function buildModelsForUnusedCPUs(this,UnusedSwCpuMdls)
            for i=1:numel(UnusedSwCpuMdls)
                ...
                if exist(swmdl_path,'file')
                    ...
                    eswElfGenPath=this.buildSoftwareApplication(swmdl);
                    [~,fn,fext]=fileparts(eswElfGenPath);
                    eswElfPrjDir=fullfile(this.ProjectDir,[fn,fext]);

                    copyfile(eswElfGenPath,eswElfPrjDir,'f');
                    if~isfield(this.socsysinfo.projectinfo,'elf_file')
                        this.socsysinfo.projectinfo.elf_file=eswElfPrjDir;
                        this.ESWElf=eswElfPrjDir;
                    else
                        this.socsysinfo.projectinfo.elf_file=[this.socsysinfo.projectinfo.elf_file,{eswElfPrjDir}];
                        this.ESWElf=[this.ESWElf,{eswElfPrjDir}];
                    end
                end
            end
        end
    ...
    methods(Static,Hidden)
        function loadAndRunExecutable(mdlName,exeFile)
            ...
            soc.ui.SoCGenWorkflow.invokeDownloader(mdlName,hCS,...
        ...
        function ret=getESWRefModel(sys)
            ...
            if isempty(taskMgrBlk)
                ...
                if~iscell(taskMgrBlk)
                    ret=soc.ui.SoCGenWorkflow.getReferenceModels(taskMgrBlk);
                else
                    ret=cellfun(@(x)soc.ui.SoCGenWorkflow.getReferenceModels(x),taskMgrBlk,'UniformOutput',false);
                end
            end
        end
        ...
        function programsoc(projectFolder)
            ...
            if hasProcessor
                ...
                soc.ui.SoCGenWorkflow.runPreProgramSoCFcn(sysinfo);

                if~soc.internal.isCustomHWBoard(hwBoard)
                    loadBitstream(hwObj,bit_file,dtb_file);
                else
                    soc.ui.SoCGenWorkflow.invokeDeployer(mdlName,false,bit_file,dtb_file);
                end
                soc.ui.SoCGenWorkflow.runPostProgramSoCFcn(sysinfo);
            else
        ...
        function runPreProgramSoCFcn(sysinfo)
            if isfield(sysinfo.ipcoreinfo,'customipcore_info')&&~isempty(sysinfo.ipcoreinfo.customipcore_info)
                ...
                for iter=1:numel(sysinfo.ipcoreinfo.customipcore_info)
                    if isfield(customipinfo(iter),'PreProgramSoCFcn')&&~isempty(customipinfo(iter).PreProgramSoCFcn)...
                        &&(isfile(which(customipinfo(iter).PreProgramSoCFcn))||isfile(customipinfo(iter).PreProgramSoCFcn))
                        mdlName=sysinfo.modelinfo.sys;
                        hwBoard=get_param(mdlName,'HardwareBoard');
                        hwObj=createHardwareObject(hwBoard);
                        if isfield(customipinfo(iter),'isUserDefined')&&(customipinfo(iter).isUserDefined)
                            run(customipinfo(iter).PreProgramSoCFcn);
                        else
                            feval(customipinfo(iter).PreProgramSoCFcn,hwObj,customipinfo(iter).PreProgramSoCFcnArgs);
                        end
                    else
        ...
        function runPostProgramSoCFcn(sysinfo)
            if isfield(sysinfo.ipcoreinfo,'customipcore_info')&&~isempty(sysinfo.ipcoreinfo.customipcore_info)
                ...
                for iter=1:numel(sysinfo.ipcoreinfo.customipcore_info)
                    ...
                    if isfield(customipinfo(iter),'PostProgramSoCFcn')&&~isempty(customipinfo(iter).PostProgramSoCFcn)...
                        &&(isfile(which(customipinfo(iter).PostProgramSoCFcn))||isfile(customipinfo(iter).PostProgramSoCFcn))
                        mdlName=sysinfo.modelinfo.sys;
                        hwBoard=get_param(mdlName,'HardwareBoard');
                        hwObj=createHardwareObject(hwBoard);
                        if isfield(customipinfo(iter),'isUserDefined')&&(customipinfo(iter).isUserDefined)
                            run(customipinfo(iter).PostProgramSoCFcn);
                        else
                            feval(customipinfo(iter).PostProgramSoCFcn,hwObj,customipinfo(iter).PostProgramSoCFcnArgs);
                        end
                    else
        ...
        function runsoftwareapp(projectFolder)
            ...
            mdlName=sysinfo.modelinfo.sys;
            hwBoard=get_param(mdlName,'HardwareBoard');
            if~soc.ui.SoCGenWorkflow.isModelProcessorOnly(mdlName)
                for i=1:numel(elfFile)
                    if~soc.internal.isCustomHWBoard(hwBoard)
                        ...
                        soc.ui.SoCGenWorkflow.loadAndRunExecutable(mdlName,elfFile{i});
                    end
                end
            else
                if~soc.ui.SoCGenWorkflow.isModelMultiCPU(mdlName)
                    soc.ui.SoCGenWorkflow.loadAndRunExecutable(mdlName,elfFile{1});
                else
                    for i=1:numel(elfFile)
                        exeFile=elfFile{i};
    ...
    methods(Hidden)
        function setHWObj(this,hwObj)
            this.SysDeployer.HWObj=hwObj;
        end
    end
end

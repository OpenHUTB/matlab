function varargout=Software2StreamCb(func,blkH,varargin)
...
function LoadFcn(blkH)
    ...
    soc.blkcb.cbutils('RegisterSetupViewerCb',blkH,'sw2stream');
end
...
function CopyFcn(blkH)
    ...
    if(codertarget.fpgadesign.internal.fpgaDesignCallback(blkH,'checkFPGACompatibility','blockInstantiation',blkPath))
        soc.blkcb.cbutils('setupViewer',blkH,'sw2stream','No debug','No debug')
    else
        ...
        if~strcmp(blkP.DiagnosticLevel,'No debug')
            soc.blkcb.cbutils('setupViewer',blkH,'sw2stream','Basic diagnostic signals','Basic diagnostic signals')
        else
            soc.blkcb.cbutils('setupViewer',blkH,'sw2stream','No debug','No debug')
        end
    end
...
function MaskInitFcn(blkH)%#ok<*DEFNU>
    ...
    try
        ...
        [ChLength,ChCompLength,~]=hsb.blkcb2.cbutils('GetChLength',blkP.ChDimensionsWriterChIf,'off');
        [~,~,~,~,chTDATASize]=hsb.blkcb2.cbutils('GetChWidths',ChType,ChCompLength);

        MRBufferSize=chTDATASize*ChLength;
        MRRegionSize=blkP.MRNumBuffers*MRBufferSize;

        [~,chOrderRd,chDimensionsRd]=hsb.blkcb2.cbutils('GetChOrder',blkP.ChDimensionsReaderChIf,blkP.ChBitPackedReaderChIf);
        ...
        'ChDimBuffer',get_param(blkH,'ChDimensionsWriterChIf'),...
        'ChTypeBuffer',get_param(blkH,'ChTypeWriterChIf'),...
...
        ...
        'ChTypeReaderChIf',ChTypeReaderChIf,...
        'ChSampleTimeReaderIf',get_param(blkH,'ChFrameSampleTimeReaderChIf'));
...
...
        'BurstLengthReaderChIf',get_param(blkH,'BurstLengthReaderChIf'),...
        'FIFODepthReader',get_param(blkH,'FIFODepthReader'),...
        'FIFOAFullDepthReader',get_param(blkH,'FIFOAFullDepthReader'),...
        'ICClockFrequencyReader',get_param(blkH,'ICClockFrequencyReader'),...
        'ICDataWidthReader',get_param(blkH,'ICDataWidthReader'),...
...
...
...
        ...
        soc.internal.setBlockIcon(blkH,'socicons.SW2Stream');
    catch ME
        hadError=true;
        rethrow(ME);
    end
...
function[vis,ens]=MemorySimulationCb(blkH,val,vis,ens,idxMap)
    ...
    soc.blkcb.cbutils('AddBadge',blkH,fullfile(badgeDir,image),tooltip,@(a,b,c)soc.internal.helpview('soc_sw_axi4_stream'));
...
function l_SetMaskHelp(blkH)
    helpcmd='eval(''soc.internal.helpview(''''soc_sw_axi4_stream'''')'')';

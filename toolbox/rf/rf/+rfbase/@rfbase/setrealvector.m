function out=setrealvector(h,out,prop_name,...
    ...
    if isempty(out)
        ...
        error(message('rf:rfbase:rfbase:setrealvector:EmptyNotAllowed',...
        rferrhole1,rferrhole2));
    end

    out=squeeze(out);
    if~isnumeric(out)||~isvector(out)||any(isnan(out))||...
        any(~isreal(out))||(~zero_included&&any(out==0))||...
        ...
        error(message('rf:rfbase:rfbase:setrealvector:NotARealVector',...
        rferrhole1,rferrhole2));
    end

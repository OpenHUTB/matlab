...
classdef CapacitorComponentView<rf.internal.apps.matchnet.ComponentView
    ...
    properties(Access=public,Constant)
        IMAGE_SERCAP='series_capacitor_60@2x.png'
        IMAGE_SHNTCAP='shunt_capacitor_60@2x.png'
    end

    methods(Access=public)

        function this=CapacitorComponentView(parent,component,connection)
        ...
        function uiobjectOut=getEditableControls(this,parent)
            ...
            uilabel(datagrid,'Text','Capacitance (pF)');


            scaledComponentValue=this.Component.Capacitance*1e12;
    ...
    methods(Access=protected)
        function setImage(this)
            if(strcmp(this.ComponentConnection,'shunt'))
                filename=fullfile(this.IMAGE_PATH,this.IMAGE_SHNTCAP);
                ComponentName=pad(regexprep(this.Component.Name,'[_]',...
                '','once'),24+numel(this.Component.Name),'left');
                HA='left';
                VA='center';
            else
                filename=fullfile(this.IMAGE_PATH,this.IMAGE_SERCAP);
            ...
            scaledComponentValue=this.Component.Capacitance*1e12;
            this.ComponentText.Tooltip=[num2str(scaledComponentValue,4),' pF'];
        end
    end
end

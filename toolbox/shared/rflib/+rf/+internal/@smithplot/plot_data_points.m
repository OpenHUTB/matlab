function plot_data_points(p)
    ...
    elseif Nd==0
        ...
        Zplane=zeros(Nd,1);
        for i=1:Nd
            Zplane(i)=getappdata(hline(i),'smithiZPlane');
        end
    ...
    for datasetIndex=1:Nd
        ...
        [x,y]=pol2cart(pdata_i.ang*(pi/180),pdata_i.mag);
        mappoints=complex(x,y);
        [th,r]=cart2pol(real(mappoints),imag(mappoints));
    ...
    hglow.LineWidth=7+2*(LineWidth(1)-1);

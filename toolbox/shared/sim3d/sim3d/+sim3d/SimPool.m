    ...
    methods(Static=true,Hidden=true)
        function reset(blk)
            if~bdIsLibrary(bdroot(blk))
                Mdl=bdroot(blk);
                [~]=sim3d.SimPool.pool('reset',Mdl,blk);
            end
        end
        ...
        function preDeleteCallback(blk)
            if~bdIsLibrary(bdroot(blk))
                ...
                if sim3d.SimPool.isCustomTag(ActorTag,ActorType)
                    [~]=sim3d.SimPool.pool('removeCustom',Mdl,blk,ActorTag);
                elseif sim3d.SimPool.exist(Mdl,blk,ActorType,ID)
                    [~]=sim3d.SimPool.pool('remove',Mdl,blk,ActorType,ID);
                end
        ...
        function addActorTag(blk)
            ...
            if~bdIsLibrary(bdroot(blk))
                ...
                ActorType=get_param(blk,'ActorType');
                ID=get_param(blk,'ID');
                switch sim3d.SimPool.getStatus(blk)
                case 'AddDefault'
                    ActorTag=sim3d.SimPool.next(Mdl,blk,ActorType);
                    set_param(blk,'ActorName',ActorTag.Tag,'ActorTag',ActorTag.Tag,'ID',ActorTag.ID);
                case 'AddNew'
                    if sim3d.SimPool.isCustomTag(ActorName,ActorType)
                        sim3d.SimPool.pool('addCustom',Mdl,blk,ActorName);
                    else
                        sim3d.SimPool.pool('add',Mdl,blk,ActorType,str2double(ID));
                    end
                case 'Refresh'
                    if sim3d.SimPool.isCustomTag(ActorName,ActorType)
                        if sim3d.SimPool.isCustomTag(ActorTag,ActorType)
                        ...
                        sim3d.SimPool.pool('add',Mdl,blk,ActorType,ID);
                ...
                otherwise
                    error('%s is not valid case',sim3d.SimPool.getStatus(blk));
                end
        ...
        function out=checkTag(Mdl,blk,ActorTag)
            ...
            if nType<length(ActorTag)&&strcmp('SimulinkVehicle',ActorTag(1:nType))
                ...
                if isnan(ID)||ID<=0||floor(ID)~=ID
                    ...
                    out=~sim3d.SimPool.pool('exist',Mdl,blk,checkType,num2str(ID));
                end
            end
        end
        ...
        function out=getStatus(blk)
            ...
            ActorType=get_param(blk,'ActorType');
            ID=get_param(blk,'ID');
            ActorBlk=sim3d.SimPool.getActorBlock(blk,ActorType,ActorTag);
            if isempty(ActorBlk)
                ActorBlk=sim3d.SimPool.getActorBlock(blk,'Custom',ActorTag);
            end

...
%#codegen
    ...
    if~isfloat(u)
        ...
        v=coder.nullcopy(localXYCastToOutType(zeros(sz),zeros(sz),xyNT,[]));
    end
    ...
    for idx=1:outLen
        ...
        if isfi(v)
            v_XY_Fm=localXYCastToOutType(xout,yout,xyNT,xyFm);
            v(idx)=v_XY_Fm;
        else
...
function v_idx=localXYCastToOutType(xout,yout,xyNT,xyFm)


    if(xyNT.FractionLength==0)



        switch(xyNT.WordLength)
        case 8
            v_idx=complex(int8(xout),int8(yout));
        case 16
            v_idx=complex(int16(xout),int16(yout));
        case 32
            v_idx=complex(int32(xout),int32(yout));
        otherwise
            v_idx=fi(complex(xout,yout),xyNT,xyFm);
        end

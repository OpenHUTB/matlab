function writeCompileDoFile(this,varargin)
    ...
    isVHDL=strcmpi(this.TargetLanguage,'vhdl');
    if isVHDL
    ...
    if strcmpi(hdlgetparameter('SynthesisTool'),'Intel Quartus Pro')&&(targetcodegen.targetCodeGenerationUtils.isALTERAFPFUNCTIONSMode())
        ...
        for n=hdlnamesStartIdx:length(hdlnames)
            if contains(hdlnames{n},libName)
                fprintf(fid,'vcom -work %s %s\n',libName,hdlnames{n});
            else
                hdlnamesNewlist=[hdlnamesNewlist,hdlnames{n}];
            end
        end
        hdlnames=hdlnamesNewlist;
    ...
    function n_ModelsimScriptSetup(isVHDL)

        [singleLib,libname,~,~,relPath]=l_getSimulatorIndependentVars(this,isVHDL);
        ...
        vmap_cEmitted=this.insertIseXSGCompileScripts(fid,vmap_cEmitted);
        vmap_cEmitted=this.insertDSPBACompileScripts(fid,vmap_cEmitted);%#ok<NASGU>
    end


    function n_IncisiveScriptSetup(isVHDL)

        [IsSingleLib,libname,CurrModeRelPath,CreateLocalLib,ChildModelsRelPath]=l_getSimulatorIndependentVars(this,isVHDL);

        IsModelRef=~isempty(this.SubModelData);

        IsSingleLib=IsSingleLib&&isVHDL;
...
function[singleLib,libname,CurrModelRelPath,CreateLocalLib,ChildModelsRelPath]=l_getSimulatorIndependentVars(this,isVHDL)

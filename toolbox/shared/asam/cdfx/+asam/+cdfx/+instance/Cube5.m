classdef Cube5<asam.cdfx.instance.Instance
...
ArrayDims
    ...
    methods
        function obj=Cube5(root,sys,inst)
        ...
        function setValue(obj,value,isInternalModify)
            ...
            for mdx=1:obj.ArrayDims(5)
                for ldx=1:obj.ArrayDims(4)
                    for kdx=1:obj.ArrayDims(3)
                        for jdx=1:obj.ArrayDims(1)
                            for idx=1:obj.ArrayDims(2)
                                ...
                                case "V"
                                    obj.ValueContainerElement.SW_VALUES_PHYS.VG(mdx).VG(ldx).VG(kdx).VG(jdx).V(idx).elementValue=string(value.PhysicalValue(jdx,idx,kdx,ldx,mdx));
                                case "VT"
                                    obj.ValueContainerElement.SW_VALUES_PHYS.VG(mdx).VG(ldx).VG(kdx).VG(jdx).VT(idx).elementValue=string(value.PhysicalValue(jdx,idx,kdx,ldx,mdx));
                                end
                            end
                        end
                    end
            ...
            for idx=1:numel(obj.SWAxisContainer)
                ...
                elseif~any(value.(fieldName),obj.Value.(fieldName))&&~isInternalModify
        ...
        function physVals=getPhysicalValues(obj)
            ...
            for outerVGIndex=1:OuterVG.Size
                ...
                MiddleVG=OuterVG(outerVGIndex).VG;


                for middleVGIndex=1:MiddleVG.Size

                    InnerVG=MiddleVG(middleVGIndex).VG;


                    for innerMiddleVGIndex=1:InnerVG.Size
                        RealInnerVG=InnerVG(innerMiddleVGIndex).VG;
                        for vgIndex=1:RealInnerVG.Size
                            ...
                            switch obj.ValueType
                            case "V"
                                V=RealInnerVG(vgIndex).V;
                            case "VT"
                                V=RealInnerVG(vgIndex).VT;
                            end
                        ...
                        finalArray(:,:,innerMiddleVGIndex,middleVGIndex,outerVGIndex)=value;%#ok<AGROW>
                        value=openString;
                    end
                end
        ...
        function resolveAxisValues(obj)
            ...
            obj.Value=struct("PhysicalValue",obj.PhysicalValue,"Axis1",axisVals{1},"Axis2",axisVals{2},"Axis3",axisVals{3},"Axis4",axisVals{4},"Axis5",axisVals{5});
        end
        ...
        function valType=getValueType(~,valueContainer)
            ...
            if~isequal(valueContainer.SW_VALUES_PHYS.VG(1).VG(1).VG(1).VG(1).V.Size,0)
                valType="V";
            else
                valType="VT";
            end

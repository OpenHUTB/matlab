classdef Cube4<asam.cdfx.instance.Instance
...
ArrayDims
    ...
    methods
        function obj=Cube4(root,sys,inst)
                ...
                for kdx=1:obj.ArrayDims(3)
                    for jdx=1:obj.ArrayDims(1)
                        for idx=1:obj.ArrayDims(2)
                            ...
                            case "V"
                                obj.ValueContainerElement.SW_VALUES_PHYS.VG(ldx).VG(kdx).VG(jdx).V(idx).elementValue=string(value.PhysicalValue(jdx,idx,kdx,ldx));
                            case "VT"
                                obj.ValueContainerElement.SW_VALUES_PHYS.VG(ldx).VG(kdx).VG(jdx).VT(idx).elementValue=string(value.PhysicalValue(jdx,idx,kdx,ldx));
                            end
                        end
                    end
                end
        ...
        function physVals=getPhysicalValues(obj)
            ...
            for outerVGIndex=1:OuterVG.Size
                ...
                for middleVGIndex=1:MiddleVG.Size
                    ...
                    finalArray(:,:,middleVGIndex,outerVGIndex)=value;%#ok<AGROW>
                    value=openString;
                end
        ...
        function resolveAxisValues(obj)
            ...
            obj.Value=struct("PhysicalValue",obj.PhysicalValue,"Axis1",axisVals{1},"Axis2",axisVals{2},"Axis3",axisVals{3},"Axis4",axisVals{4});
        end
        ...
        function valType=getValueType(~,valueContainer)
            ...
            if~isequal(valueContainer.SW_VALUES_PHYS.VG(1).VG(1).VG(1).V.Size,0)
                valType="V";
            else
                valType="VT";
            end

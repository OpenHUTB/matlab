function[ZL,loadedge]=loadingedge(obj,~,frequency,...
    ...
    loadedge=0;ZL=50;
    if~isfield(obj.MesherStruct,'Load')||isempty(obj.MesherStruct.Load)
        return;
    elseif obj.MesherStruct.Load.numLoads==0&&addtermination==0
        return;
    end


    if isa(obj,'em.Array')&&isprop(obj,'Element')&&...
        isa(obj.Element,'em.Array')&&~addtermination
        return;
    end
    ...
    if addtermination
        ...
        if all(loadedge==0)
            ...
            if isempty(index)
                ...
                Zval=Zterm*ones(1,numel(terminatingedge));
                if strcmpi(obj.MesherStruct.Mesh.FeedType,'doubleedge')
                    Zval=Zval*2;
                end
                ZL=[ZL,Zval];

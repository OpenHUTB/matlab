function metalbasis=findLitRegionsEm(metalbasis,P,t,idPO,feededge,mode)
    ...
    for k=1:N_angs
        ...
        if emn
            ...
            if~isempty(idTri)
                TR=triangulation(t(1:3,idTri)',P');
                rt=matlabshared.internal.StaticSceneRayTracer(TR);
                [dirn,dists]=matlabshared.internal.segmentToRay(FeedCenter',C);
                [~,litID,~]=firstIntersection(rt,FeedCenter',dirn,10*dists);
                litID=litID';
            end
        else
            for i=1:size(O,1)

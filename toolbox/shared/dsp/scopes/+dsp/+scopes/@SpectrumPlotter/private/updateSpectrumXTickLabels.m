function updateSpectrumXTickLabels(this)




    hAxes=this.Axes;
    if this.ShowXAxisLabels&&~isempty(hAxes)&&strcmp(hAxes(1).Visible,'on')
        if strcmp(this.pXTickLabelMode,'auto')||isempty(this.pXTickLabel)
            ...
            xTicks=get(hAxes(1,1),'XTick')*this.FrequencyMultiplier;
        ...
        if(length(xTicks)>1)&&(length(cellstr(xTickLabels))~=length(unique(cellstr(xTickLabels))))
            ...
            set(hAxes(1,1),'XTickLabel',xTickLabels);
        end
    end
end

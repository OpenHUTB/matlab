function makeSymbolTable(this,d,out,s)
    ...
    getString(message('RptgenSL:csl_emlfcn:symbolTableName')));
    pi=createProcessingInstruction(d,'db2dom','style-name="Heading 2"');
    appendChild(elem,pi);
    appendChild(out,elem);

    nArgs=numel(s);
    nProps=length(this.SymbolTableProps);
    tableArray=cell(nArgs+1,nProps);

    for iProp=1:nProps
        tableArray{1,iProp}=this.SymbolTableColHeaders(iProp);
    end
    ...
    for iArg=1:nArgs
        ...
        elseif all(s(iArg).size==1)
            ...
            dataType=strcat(s(iArg).dataType,' [',int2str(s(iArg).size),']');
        end

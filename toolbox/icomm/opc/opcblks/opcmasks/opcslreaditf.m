function varargout=opcslreaditf(block,action,varargin)
    ...
    function RefreshClientList(block,handles,clntList)
        ...
        if strcmp(get(handles.popClient,'Enable'),'on'),
            ...
            readBlk=handles.blockHandle;
            if isempty(readBlk.serverHost),
                clientStr='';
            else
                clientStr=sprintf('%s/%s',...
                readBlk.serverHost,readBlk.serverID);
            end
        end
        ...
        function myDlg=GetOpenBlockDlg(block)
            ...
            myDlg=[];
            allDlg=findall(0,'Tag','dlgOPCRead');

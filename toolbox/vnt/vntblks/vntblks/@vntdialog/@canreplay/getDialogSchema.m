...
function paramPane=localCreateParamGroup(obj,allDevices)
    ...
    rowInDialog=rowInDialog+1;
    colSpan=[1,20];
    BusOptionCheckbox=tamslgate('privateslwidgetcheckbox',...
    widgetPrompts.BusOutputPrompt,widgetTags.BusOutput,...
    [rowInDialog,rowInDialog],colSpan,'vntslshared.internal.maskCallback');
    BusOptionCheckbox.Mode=true;
    BusOptionCheckbox.DialogRefresh=false;
    ...
    function nestedDisableWidgets()
        ...
        if strcmpi(obj.ReplayTo,'CAN Bus')
            DeviceMenuField.Enabled=true;
            BusOptionCheckbox.Enabled=false;
        else
            BusOptionCheckbox.Enabled=true;
    ...
    SampleTimeField,SampleTimeText,BusOptionCheckbox};

function toolChainConfiguration=ccev5Decorator(data)
    ...
    function files=libraryFilesOverride(~,codeGenFiles,data)
        ...
        for index=1:length(codeGenFiles)
            ...
            if(regexpi(codeGenFiles{index},rtsLibWithCCSv3RelPath))
                ...
                linkfoundation.xmakefile.XMakefilePreferences.(['getCCEv5',upper(proc.family),'CGToolsInstallLocation'])().Path,...
                'lib',rtsLibFile);
            end
            ...
            if(regexpi(codeGenFiles{index},cslLibWithCCSv3RelPath))
                ...
                cslLibDir=linkfoundation.xmakefile.XMakefilePreferences.(['getCCEv5',upper(proc.family),'CSLInstallLocation'])().Path;
    ...
    function flags=baseCodeGenLinkerFlagsOverride(~,codeGenFlags,data,proc)
        if(strcmpi(linkfoundation.xmakefile.XMakefileConfigurationEvent.EXECUTABLE_TARGET_BEFORE_BUILD,data.Context))
            ...
            cgtLibLoc=linkfoundation.xmakefile.XMakefilePreferences.getCCEv5CGToolsLibPath(proc);
            if~isempty(cgtLibLoc)
                cgtLibPath=linkfoundation.util.Location.convertToUnixPath(cgtLibLoc.Path);
                flags=sprintf('%s %s',flags,['-I"',cgtLibPath(1:end-1),'"']);
            end
            if strcmpi(proc,'c2000')
                cslLibLoc=[];
            else
                cslLibLoc=linkfoundation.xmakefile.XMakefilePreferences.getCSLLibPathForCCEv5(proc);
            end
